---
title: "Yarah FAQ: databases, schemas, edge functions, and SDK"
sidebarTitle: "FAQ"
description: "Answers to common Yarah questions on database calls, edge functions, custom compute, querying non-public schemas from the SDK, and RLS."
---
...
<AccordionGroup>
  <Accordion title="Is reading or writing the database an edge function? What's the difference between database calls, edge functions, and custom compute?">
    ...
    In Yarah your code talks to the backend in three different ways, and they're easy to mix up:
    ...
    **Database call.** Define a table and Yarah instantly gives you a set of REST endpoints (like `GET /api/database/records/{table}`) and a typed SDK. Calling `select` or `insert` reads and writes the database directly, with nothing to deploy and nothing running. This is all you need for ordinary create/read/update/delete. See [Database](/core-concepts/database/overview).
  ...
  <Accordion title="How do I query a table in a schema other than `public`?">
    ...
    **CLI.** The CLI reads and writes any schema through `db query`, just schema-qualify the table:
    ...
    ```bash
    yarah db query "SELECT * FROM my_schema.mytable"
    ```

    **SDK.** Chain `.schema()` before the query builder (supported by `@yarahdev/sdk`). It maps to the same `Accept-Profile` / `Content-Profile` header, so reads, writes, and RPC all route to the schema you name:
  ...
  <Accordion title="How do I turn row-level security (RLS) on or off for a table?">
    ...
    ```bash
    # one-off, via the CLI
    yarah db query "ALTER TABLE public.mytable DISABLE ROW LEVEL SECURITY"

    # or track it as a migration
    npx @yarah/cli db migrations new disable-rls-on-mytable
    # put the ALTER TABLE statement in the generated .sql file, then:
    npx @yarah/cli db migrations up --all
    ```
  ...
  <Accordion title="Does Yarah have a `service_role` key / `YARAH_SERVICE_ROLE_KEY`?">
    Not under that name. Yarah's equivalent is your project's **API Key** (it starts with `ik_`), the full-access admin key. Every project has two keys:
    ...
    Find the API Key in the dashboard under **Project Settings → General** (the **API Key** row, marked "full access control... do not expose in your frontend"), or run `npx @yarah/cli secrets get API_KEY`.
    ...
    const admin = createAdminClient({
      ...
      apiKey: process.env.YARAH_API_KEY, // admin key (ik_...), bypasses RLS
    })
  ...
  <Accordion title="How do I share a project with another admin, or invite a teammate?">
    ...
    4. They get an email invite that is valid for 7 days. When they sign in to Yarah **with that same email address** and accept it, they join the organization with the role you chose.
    ...
    Handing the organization over to a new **Owner** entirely is a separate action from inviting members. To do that, open **Organization Settings** and use **Transfer Ownership** (only the current owner can start it, and the recipient must be a verified Yarah user who accepts the emailed request).
  </Accordion>
  ...
  <Accordion title="How do I authenticate the CLI without a browser?">
    `npx @yarah/cli login` opens a browser to sign in. On a headless machine, a remote server, or CI, use a user API key instead. No browser needed.
    ...
    ```bash
    npx @yarah/cli login --user-api-key <your-user-api-key>
  ...
  <Accordion title="What is FLY_API_TOKEN?">
    It's an environment variable you only set when you self-host Yarah and want to use [Custom Compute](/core-concepts/compute/overview). Custom Compute runs your long-lived containers on [Fly.io](https://fly.io), so a self-hosted instance needs your own Fly account: set `FLY_API_TOKEN` (a Fly API token from `fly tokens create org`) and `FLY_ORG` (your Fly org slug from `fly orgs list`) in your `.env`, then restart. Both are required, and until they're set, compute endpoints return `503 COMPUTE_NOT_CONFIGURED`.

    On Yarah Cloud you never touch this. Compute is managed for you, and the rest of the platform (database, auth, storage, edge functions) needs no Fly token at all.
  </Accordion>

  <Accordion title="Can this assistant help with something specific to my own project?">
    Not really. This assistant answers from Yarah's public docs, so it can't see your project: it can't debug an error, read your data, or check your configuration. Take anything specific to your own project to your coding agent instead. Connected to Yarah through the CLI or MCP, your agent can read your live backend, schema, data, and logs and debug the problem directly. Just describe it in plain words. For a backend health and error report you can run yourself, use `npx @yarah/cli diagnose`. See [Diagnostics & advisor](/agent-native/diagnostics).
  </Accordion>
  ...
  <Accordion title="How do I get my database's Postgres connection string?">
    ...
    ```bash
    npx @yarah/cli db connection-string
    ```
    ...
    ```text
    postgresql://postgres:<password>@<appkey>.<region>.database.apps.yarah.dev:5432/yarah?sslmode=require
    ```
  ...
  <Accordion title="How do I import or load an existing Postgres database into Yarah?">
    Restore your existing database over your project's Postgres connection string (the same URL from the question above). Standard PostgreSQL client tools — `pg_dump`, `pg_restore`, and `psql` — connect to it directly, so an import is just a dump-then-restore. No special Yarah command is involved.
    ...
    2. Get your Yarah connection string (cloud projects only):

    ```bash
    npx @yarah/cli db connection-string
    ```

    3. Restore into Yarah:

    ```bash
    # Custom-format dump
    pg_restore --no-owner --single-transaction -d "postgresql://postgres:<password>@<appkey>.<region>.database.apps.yarah.dev:5432/yarah?sslmode=require" local.dump

    # ...or a plain SQL file
    psql --single-transaction "postgresql://postgres:<password>@<appkey>.<region>.database.apps.yarah.dev:5432/yarah?sslmode=require" < local.sql
    ```
    ...
    - Imported tables do **not** get Yarah's managed access automatically. A raw restore skips the `anon` / `authenticated` grants and row-level security Yarah applies, so imported tables are **not** reachable through the REST API or SDK — and any access rules from your source dump don't carry Yarah's RLS protection — until you grant access and add RLS policies for each table (see [Database](/core-concepts/database/overview)). If you'd rather version schema changes in git, see [Database migrations](/core-concepts/database/migrations).
  </Accordion>

  <Accordion title="How do I take down or remove a deployed site?">
    There's currently no self-serve way to take down a deployed [Site](/core-concepts/sites/overview) — there's no `deployments delete` command and no dashboard action for it. Deployed sites are hosted externally, so even deleting your project (`npx @yarah/cli projects delete --project <id>`) tears down your backend resources — database, storage, and backend branches — but does *not* remove the hosted site.

    In practice this rarely matters. If you do need a deployed site taken down, ask the Yarah team in [Discord](https://discord.com/invite/DvBtaEc9Jz).

    Two related actions that are *not* the same as removing a live site:

    - **Cancel a build that's still running:** `npx @yarah/cli deployments cancel <id>` stops an in-progress deployment; it does not take down a site that is already live.
    - **Replace what's live:** redeploy over the same site with `npx @yarah/cli deployments deploy ./frontend` — the newest ready deployment serves the URL.
  </Accordion>
</AccordionGroup>
