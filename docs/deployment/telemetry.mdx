...
description: "Learn what anonymous usage data Yarah self-hosted deployments send, what is never collected, and how to opt out using YARAH_TELEMETRY_DISABLED."
---

Yarah collects anonymous telemetry from self-hosted deployments to understand active installs, version adoption, deployment methods, and which optional services are configured. This helps maintainers prioritize fixes, security notices, and deployment work for the open-source project.

Telemetry is optional. You can disable it at any time.
Yarah does not send this telemetry from Yarah Cloud.
...
Yarah sends an `oss_instance_started` event when the backend starts and an `oss_heartbeat` event about once every 24 hours while the backend is running.
Events are sent through Yarah's PostHog proxy at `https://b.yarah.dev/capture/` using PostHog's capture API format.

Each event contains:

- PostHog event name: `oss_instance_started` or `oss_heartbeat`
- Anonymous installation ID stored locally in `LOGS_DIR/.yarah-installation-id`, used as the PostHog `distinct_id`
- Yarah version
- Event timestamp
...
- On heartbeat events only, the list of features touched since the previous heartbeat, such as `["auth", "database", "storage"]`. This is a yes or no per feature. Yarah does not send request counts, request volumes, paths, request contents, or anything about who made the request. Reads issued from an open dashboard tab, dashboard sign-in traffic, and unauthenticated requests are not treated as usage.
...
Set `YARAH_TELEMETRY_DISABLED=1` in your environment and restart the backend.
For Docker Compose deployments, add the same line to your `.env` file:

```bash
YARAH_TELEMETRY_DISABLED=1
```
