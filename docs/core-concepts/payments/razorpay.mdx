...
description: "Integrate Razorpay Orders, Items, Plans, and Subscriptions with Yarah: server-side signature verification, mirrored tables, and webhook fulfillment."
---
...
| Razorpay concept | Meaning in Yarah |
|------------------|---------------------|
...
Razorpay does not have a Stripe Billing Portal equivalent. Yarah exposes backend routes to cancel, pause, and resume subscriptions after checking the caller against `payments.razorpay_subscriptions` RLS `UPDATE` policies.

For one-time products, Razorpay Orders can be created with only an amount, currency, and receipt. Still, prefer creating Razorpay Items for sellable products so the catalog is visible in Yarah and Razorpay after sync. Treat Orders as payment attempts, not as your product catalog.

## Setup

Configure `test` and `live` Razorpay Key ID and Key Secret in Dashboard -> Payments -> Settings or through the admin API. Yarah generates a webhook signing secret if one does not already exist.
...
4. Select the events Yarah handles.
...
Create an app-owned pending order first. Then create a Razorpay Order with the current Yarah user token.
