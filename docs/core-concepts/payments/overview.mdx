...
description: "Collect money in your app with your own Stripe or Razorpay account. Pick the provider-specific Yarah Payments integration for checkout and subscriptions."
---

Yarah Payments lets your app collect money with your own Stripe or Razorpay account. The two providers use different payment models, so the docs are split by provider instead of describing one generic payment flow.

<Frame caption="Payments dashboard: provider connection, catalog, subscriptions, and transactions.">
  <img src="/images/dashboard-payments.png" alt="Yarah Payments dashboard" />
</Frame>

<Note>
  Stripe or Razorpay remains the source of truth for charges, invoices, refunds,
  disputes, taxes, and account-level financial operations. Yarah is not a
...
| `payments.transactions` | Yarah dashboard/reporting projection for successful, failed, and refunded payment activity. |
