...
Migrations are versioned SQL files in `migrations/` applied with `@yarahdev/cli`. Each successful run is recorded in `system.custom_migrations`. The workflow is forward-only.
