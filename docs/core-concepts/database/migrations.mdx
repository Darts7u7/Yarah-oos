...
description: "Track Postgres schema changes as timestamped SQL files in git and apply them forward-only with the Yarah CLI, recording history for every project."
---

Migrations are versioned SQL files in `migrations/` applied with `@yarah/cli`. Each successful run is recorded in `system.custom_migrations`. The workflow is forward-only.
