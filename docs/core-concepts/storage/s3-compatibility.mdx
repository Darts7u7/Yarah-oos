...
description: Use any AWS SigV4 client — aws CLI, rclone, boto3, Terraform — against Yarah Storage through the S3-compatible gateway at /storage/v1/s3.
---

Yarah Storage speaks the [AWS S3 protocol](https://docs.aws.amazon.com/AmazonS3/latest/API/Welcome.html) at `/storage/v1/s3`. Available on cloud projects and on self-hosted deployments with an S3-backed storage provider — see [Self-hosted storage](/deployment/self-host-storage).
...
[profile yarah]
region = us-east-2
endpoint_url = https://project_ref.region.apps.yarah.dev/storage/v1/s3
s3 =
...
const client = new S3Client({
  forcePathStyle: true,
  region: 'us-east-2',
  endpoint: 'https://project_ref.region.apps.yarah.dev/storage/v1/s3',
