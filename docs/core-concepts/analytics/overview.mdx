...
description: "Wire PostHog to your Yarah project to see traffic, retention, and session replay dashboards for your app without leaving the Yarah dashboard."
---

Use Yarah Analytics to understand how people actually use your app: page traffic, retention, and session replays, all wired up by connecting a PostHog project to your Yarah backend. Once connected, the dashboard renders Traffic, User Retention, and Session Replay pages on top of your PostHog data without leaving Yarah.
...
<Frame caption="Analytics dashboard: KPIs over time plus breakdowns by page, country, and device.">
  <img src="/images/dashboard-analytics.png" alt="Yarah Analytics dashboard showing visitor KPIs, a visitor trend chart, and top pages, countries, and devices breakdowns" />
</Frame>

<Note>
  PostHog remains the source of truth for events, dashboards, insights, and recordings. Yarah surfaces a focused subset for everyday checks, then deep-links into PostHog for anything beyond it.
</Note>
...
Connect PostHog from the Analytics page in the dashboard. Yarah provisions or links a PostHog project for you, stores credentials server-side, and unlocks the Traffic, Retention, and Session Replay pages once the connection succeeds.
...
The Analytics Config dialog (the gear icon in the sidebar) lets admins review the linked PostHog project, jump straight into PostHog, and disconnect when needed. Disconnecting only severs the Yarah ↔ PostHog link; your PostHog project, events, and recordings stay intact.
...
<CardGroup cols={2}>
  ...
  <Card title="Yarah CLI" icon="terminal" href="/quickstart">
    `npx @yarahdev/cli posthog setup` links your Yarah project to PostHog, then prints the wizard command.
  </Card>
</CardGroup>
