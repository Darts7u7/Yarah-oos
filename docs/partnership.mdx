...
description: Integrate Yarah as a co-branded or white-label partner platform with one-click connections, project lifecycle APIs, and dashboard embedding.
---

## Overview

Yarah provides two partnership models that enable seamless integration of our Backend-as-a-Service platform into your applications. Whether you're looking for a co-branded solution or a fully white-labeled experience, we have the right partnership model for you.

## Partnership Models

<CardGroup cols={2}>
  <Card title="Co-Branded Partnership" icon="handshake">
    Perfect for platforms that want to offer Yarah alongside their services.
    The integration process is remarkably simple - developers can connect to the Yarah platform with just one click, without complicated OAuth flows.
  </Card>
...
<Steps>
  <Step title="Apply for Partnership">
    Submit your partnership application through our [partner email](mailto:partnerships@yarah.dev).
...
### Co-Branded Features
In co-branded mode, developers explicitly know they are Yarah platform users (linked through the same email address). After logging into the Yarah platform, developers have full management rights for projects created through partners and need to pay according to Yarah's billing plans.

Partner platforms can:
- Sync user accounts (name, email) with Yarah
- Sync projects to Yarah
- Query project connection information to leverage the completed backend capabilities.

### White-Label Features
In white-label mode, developers are not aware of Yarah's existence and cannot see partner-created projects on the Yarah platform.
...
- Restart projects (optionally upgrade the Yarah OSS version)
- Delete projects
...
<Tabs>
  <Tab title="Co-Branded APIs">
    ...
    Synchronize user account information with Yarah.
    ...
    **Response:**
    ...
    Note: Due to project limits on the Yarah free plan, project synchronization from partner platforms may fail. In such cases, the response will be:
  ...
  <Tab title="White-Label APIs">
    ...
    **Response:**
    ...
    ### Restart Project
    Restart a project, optionally upgrading its Yarah OSS version. Omit `yarah_oss_version` to restart on the current version.
    ...
    **Request Body:**
    ```json
    {
      "yarah_oss_version": "v2.2.1",  // optional — omit to keep the current version
      "wait_for_completion": false        // optional
    }
    ```
...
Yarah provides global deployment capabilities. Each project can be deployed to different regions:
...
Based on your project's business requirements, Yarah offers the following resource types:
...
- Schedule a [technical review](https://calendly.com/tony-chang-yarah/45min) with our team