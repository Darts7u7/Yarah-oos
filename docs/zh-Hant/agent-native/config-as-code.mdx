...
<Note>
  所有範例使用 `npx @yarahdev/cli`。不要全域安裝 CLI。
</Note>
...
- **自託管和本機 OSS 開發。** 執行 `npx @yarahdev/cli link --api-base-url http://localhost:7130 --api-key <local-key>` 針對 docker-compose 堆疊，然後將 CLI 的設定命令指向您的本機 OSS 實例，方式與您指向雲的方式相同。
