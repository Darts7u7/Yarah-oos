---
title: "將 Yarah 部署到 Containarium"
description: "在 Containarium LXC 伺服器上執行 Yarah，透過每租戶容器、ZFS 快照與 MCP 驅動設定，實現代理原生部署。"
---

# 將 Yarah 部署到 Containarium
...
如果你需要以下特性，Containarium 非常適合 Yarah 部署：

- **自架的多租戶基礎設施**：在一台伺服器上執行多個相互隔離的 Yarah 專案，每個專案都在自己的 LXC 中，每個專案有一個 TLS 主機名稱——無需共用 `docker compose -p` 記帳。
...
每個 Yarah box 的最低規格：**2 個 vCPU、4 GB 記憶體、30 GB 磁碟**。
...
`--stack docker` 旗標會在容器內安裝 Docker CE 和 compose 外掛程式。設定你的 SSH，使 `ssh yarah` 可以運作：
...
完整清單（OpenRouter、OAuth 提供者、Stripe、Vercel）請參閱 [`.env.example`](https://github.com/yarah/yarah/blob/main/.env.example)。
...
你應該會看到 `4/4 services up`：`postgres`、`postgrest`、`yarah`、`deno`。（compose 檔案為 `postgres`、`postgrest` 和 `deno` 提供了健康檢查；`yarah` 會在其他服務健康且自身已啟動後回報 `Up`。）

### 5. 透過公開主機名稱對外公開

Yarah 預設在 7130 埠提供儀表板和 API 服務。
...
這會將 Containarium sentinel 上的 Caddy 設定為終止 `<your-subdomain>` 的 TLS 並轉發到 Yarah 容器。憑證會在第一次請求時透過 ACME 自動配置——無需 certbot，無需 nginx 設定。
...
### 6. 將你的代理連接到 Yarah MCP

在瀏覽器中開啟 `https://<your-subdomain>`，依照產品內的流程將你的支援 MCP 的代理（Cursor、Claude Code、Windsurf、OpenCode 等）連接到 Yarah MCP 伺服器。
...
或依服務檢視：`docker compose logs -f yarah` / `postgres` / `deno`。
...
Yarah 的四個服務在閒置時大約需要 3 GB 常駐記憶體。如果你將 box 設定為 2 GB，請調整大小：
...
- **GPU 直通**：Containarium 支援它，但 Yarah 內建的邊緣函數不使用 GPU。除非你的自訂 Deno 函數需要 GPU，否則請保持關閉。
...
- 對於正式環境，請選擇使用 Containarium 的 KMS 信封加密（Vault Transit 或 GCP KMS）來保護儲存在 Containarium 密鑰庫中的任何 Yarah 密鑰。
