---
title: 身份驗證 SDK 參考
description: "使用 Yarah Kotlin SDK 處理使用者註冊、登入、OAuth、工作階段管理與個人資料設定，適用於 Android 與 KMP。"
---
...
對於使用電子郵件註冊的使用者，Yarah 後端提供三種選項：

1. **無電子郵件驗證** - 使用者可在註冊後立即登入。`SignUpResponse` 將具有 `accessToken != null`。
2. **基於連結的驗證** - 使用者必須開啟電子郵件並按一下驗證連結，然後才能登入。
3. **基於代碼的驗證** - Yarah 後端向使用者的電子郵件傳送 6 位驗證代碼。用戶端應用程式需要顯示驗證畫面，使用者可以在其中輸入代碼，然後呼叫 `verifyEmail(email, code)` 以完成驗證。只有之後使用者才能使用電子郵件 + 密碼登入。
...
- `redirectUri` (`String`) - 回呼 URL，Yarah 將在身份驗證後重新導向到此 URL
...
建立 Yarah 用戶端時，設定 `browserLauncher` 以處理開啟 URL：
...
`client` 應來自您在應用程式中其他地方使用的同一共用 Yarah 用戶端執行個體，例如 `Application` 單一形式或您的 DI 容器。
