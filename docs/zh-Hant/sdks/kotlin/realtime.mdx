...
description: "使用 Yarah Kotlin 即時用戶端透過 Socket.IO 連線、訂閱頻道、發佈事件、追蹤線上狀態並自動處理重新連線。"
---
...
Kotlin SDK 透過 Socket.IO 連線到 Yarah 即時系統。訂閱頻道名稱（如 `order:123` 或 `chat:room-1`），然後監聽發佈到這些頻道的事件名稱。
