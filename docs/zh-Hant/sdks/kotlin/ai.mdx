---
title: 模型閘道
description: "使用 Yarah 預配的 OpenRouter 金鑰，從 Kotlin 應用程式呼叫 LLM 建構聊天、摘要與嵌入等 AI 功能。"
---

## 概述

Yarah 為模型閘道專案預配了一個 OpenRouter API 金鑰。新的 Kotlin 應用程式應從受信任的伺服器端程式碼、後端 API 或其他安全邊界呼叫 OpenRouter。不要將 OpenRouter 金鑰嵌入 Android 或桌面用戶端二進位檔案中。

以前的 Yarah Kotlin AI SDK 方法已被棄用為相容性包裝函式。使用 Yarah SDK 來實現資料庫、身份驗證、儲存空間、函式和即時功能；使用 OpenRouter 來進行模型呼叫。
...
它們針對已棄用的 Yarah AI 代理。新的整合應使用儀表板中的 OpenRouter 金鑰，並遵循 OpenRouter 目前的 API 文件來瞭解模型參數和功能。
