...
description: "從 TypeScript 應用程式建立 Razorpay 訂單與訂閱，處理 webhook 履行並將付款狀態同步回 Yarah 資料庫。"
---
...
SDK 使用目前 Yarah 使用者令牌呼叫 Razorpay 執行時路由。不要使用提供商密鑰呼叫這些路由。
...
首先建立一個應用擁有的待處理訂單。然後透過 Yarah 建立 Razorpay 訂單：

Razorpay 訂單可以是僅金額的，但為一次性可售產品建立 Razorpay 商品是一個好做法，因為同步的商品使目錄在 Yarah 和 Razorpay 中可見。將訂單視為付款嘗試。
...
首先建立或同步一個 Razorpay 計劃。然後透過 Yarah 建立訂閱：
