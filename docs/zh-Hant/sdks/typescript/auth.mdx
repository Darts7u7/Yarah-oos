---
title: 身分驗證 SDK 參考
description: "使用 Yarah TypeScript SDK 處理使用者註冊、登入、OAuth 提供者、工作階段與個人資料設定於 Web 與 Node。"
---
...
- `redirectTo` (string, optional) - 用於連結型電子郵件驗證。電子郵件連結一律首先開啟 Yarah 後端端點；在驗證令牌後，Yarah 將瀏覽器重新導向至此 URL 以取得驗證結果。當 `verifyEmailMethod` 設為 `link` 時需要。此 URL 必須包含在 `allowedRedirectUrls` 中。建議：使用您的應用登入頁面。
...
<Note>
  當 `requireEmailVerification` 為真時，`accessToken` 將為空直到使用者驗證其電子郵件。Yarah 傳送驗證電子郵件，其中包含連結或 6 位數字代碼，具體取決於您的儀表板設定（`verifyEmailMethod`）。如果是代碼驗證，實作提示使用者輸入代碼的頁面（見 [verifyEmail()](#verifyemail)）。如果是連結驗證，提供在 Yarah 驗證令牌後應接收瀏覽器的 `redirectTo` URL。建議：使用您的登入頁面作為 `redirectTo`，然後顯示成功訊息並詢問使用者使用其電子郵件和密碼登入。
</Note>
...
<Note>
  OAuth 重新導向後，SDK 自動偵測回呼 `yarah_code`，將其交換為工作階段，並自動儲存工作階段。
</Note>
...
<Note>
  `additionalParams` 僅用於提供商特定的選用提示。不要傳遞伺服器擁有的 OAuth 欄位如 `client_id`、`redirect_uri`、`code_challenge`、`state`、`response_type` 或 `scope`；Yarah 在伺服器上設定這些值並忽略衝突的用戶端提供的索引鍵。
</Note>

<Note>
  自訂提供商必須首先在 Yarah 儀表板的 `Auth Methods` 中使用用戶端認證和 OIDC 探索 URL 進行設定。
</Note>
...
當之前的 OTP 已過期或未收到時，重新傳送電子郵件驗證。使用身分驗證設定中設定的方法（`verifyEmailMethod`）。當方法為 `code` 時，傳送 6 位數字代碼。當方法為 `link` 時，傳送透過 Yarah 後端端點的瀏覽器驗證連結。
...
- `yarah_type`：流識別碼。對於驗證連結，這一律是 `verify_email`。
- `yarah_error`：僅當 `yarah_status=error` 時存在。人類可讀的錯誤訊息用於顯示或記錄。

建議的處理方式：使用您的登入頁面作為 `redirectTo`。當 `yarah_status=success` 時，顯示確認訊息並詢問使用者使用其電子郵件和密碼登入。
...
使用身分驗證設定中設定的方法（`resetPasswordMethod`）傳送密碼重設電子郵件。當方法為 `code` 時，傳送 6 位數字代碼用於兩步流程。當方法為 `link` 時，傳送透過 Yarah 後端端點的瀏覽器重設連結。
...
- `email` (string, required) - 使用者的電子郵件地址
- `redirectTo` (string, optional) - 用於連結型密碼重設。電子郵件連結一律首先開啟 Yarah 後端端點；Yarah 然後將瀏覽器重新導向至此 URL，在查詢字串中包含重設 `token`，以便您的應用可以呈現其自己的重設密碼頁面。當 `resetPasswordMethod` 設為 `link` 時需要。此 URL 必須包含在 `allowedRedirectUrls` 中。建議：使用您的應用的專用重設密碼頁面。
...
- `token`：僅當 `yarah_status=ready` 時存在。將此值傳遞給 `resetPassword({ otp })`。
- `yarah_status`：瀏覽器連結流的結果。對於重設連結，值為 `ready` 或 `error`。
- `yarah_type`：流識別碼。對於重設連結，這一律是 `reset_password`。
- `yarah_error`：僅當 `yarah_status=error` 時存在。人類可讀的錯誤訊息用於顯示或記錄。

僅當 `yarah_status=ready` 且 `token` 存在時才呈現重設密碼表單。
