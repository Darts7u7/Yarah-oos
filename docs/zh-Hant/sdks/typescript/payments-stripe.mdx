---
title: Stripe 支付 SDK
description: "使用 Yarah TypeScript SDK 建立 Stripe Checkout 與帳單入口工作階段，同步訂閱狀態並處理 webhook 履行。"
---
...
SDK 方法需要目前 Yarah 使用者令牌。匿名 Yarah 令牌可用於來賓一次性結帳，但 Yarah API 密鑰不能替代，因為後端需要使用者內容來處理本機工作階段列。

## createCheckoutSession()

透過 Yarah 後端建立 Stripe Checkout 工作階段。
...
如果一次性結帳包含 `subject` 且還沒有現有的 Stripe 客戶對應，Yarah 讓 Stripe 在 Checkout 期間建立客戶，並從完成 webhook 回填主體對應。
...
SDK 方法使用目前 Yarah 令牌呼叫執行時路由。後端使用呼叫者內容插入本機工作階段列：
...
<Warning>
  永遠不要將 Stripe 密鑰放在前端程式碼或公開部署環境變數中。透過 Yarah 儀表板或 CLI 設定 Stripe 密鑰。
</Warning>
