...
description: "使用 Yarah TypeScript SDK 透過 Socket.IO 訂閱頻道、發佈事件、追蹤線上狀態，並自動處理重新連線與權杖更新。"
---
...
TypeScript SDK 向您的 Yarah 後端開啟一個 Socket.IO 連線。您訂閱具名頻道、監聽事件名稱，並可選擇性地將事件發佈回您已加入的頻道。

事件可以來自兩個地方：

- 呼叫 `realtime.publish(channel, event, payload)` 的資料庫觸發器。
- 呼叫 `yarah.realtime.publish(channel, event, payload)` 的用戶端。
