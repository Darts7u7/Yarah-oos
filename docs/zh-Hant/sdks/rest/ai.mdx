---
title: AI REST 參考
description: "直接透過 OpenRouter HTTP 整合並使用 Yarah 模型閘道輔助端點呼叫 LLM，統一管理金鑰、配額與計費。"
---

## 概覽

新的 AI 整合應直接呼叫 OpenRouter，使用由 Yarah 提供的 OpenRouter 密鑰。Yarah AI 代理路由（用於聊天、影像生成和嵌入）是已棄用的相容性端點。
...
在 `Authorization` 標頭中使用管理員 JWT 持有者令牌或 Yarah API 密鑰持有者令牌。
...
此路由驗證 Yarah 格式的請求，將其轉送到 OpenRouter，並傳回 Yarah 格式的回應。它不公開完整的 OpenRouter API 表面。
...
此路由將 Yarah 格式的影像請求轉送到 OpenRouter 聊天完成（帶影像輸出）。僅為了相容性而保留。
...
此路由將 Yarah 格式的嵌入請求轉送到 OpenRouter。對於新代碼，更偏好直接 OpenRouter 嵌入端點。
