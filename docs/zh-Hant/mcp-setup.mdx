...
description: "透過 MCP 將 Cursor、Claude、Windsurf 等編輯器連接至 Yarah，讓 AI 代理可查詢資料庫、身份驗證、儲存與函式。"
---

## 概觀

Yarah MCP 伺服器讓你的 AI 程式碼助手可以直接存取你的後端：資料庫查詢、結構描述管理、儲存空間等等。

請依照以下針對你所使用 AI 用戶端的說明操作。找不到你的工具？瀏覽[代理目錄](https://yarah.dev/agents)查看完整的已測試整合清單。

## 先決條件

- 一個 **AI 程式碼助手**（Cursor、Claude Code、GitHub Copilot 等）
- 一個 **Yarah 專案** - 如果還沒有，[點此建立](https://yarah.dev)
...
<AccordionGroup>
  <Accordion title="Cursor">
    ...
    4. 貼上來自你 Yarah 專案的 MCP JSON
    5. 儲存設定檔
    6. 你應該會看到一個綠點，表示 Yarah 已成功安裝
    ...
    如果連接成功，你會在 Yarah 儀表板（右上角）看到 **Connected** 狀態指示。
  ...
  <Accordion title="Claude Code">
    ...
    將 Yarah MCP 伺服器新增到你的 **Claude Code**：
    ...
    如果連接成功，你會在 Yarah 儀表板（右上角）看到 **MCP Connected** 狀態指示。
  ...
  <Accordion title="GitHub Copilot">
    ...
    將 Yarah MCP 伺服器新增到你的 **GitHub Copilot**：
  ...
  <Accordion title="Google Antigravity">
    ...
    將 Yarah MCP 伺服器新增到你的 **Antigravity**：
  ...
  <Accordion title="Trae">
    ...
    此外，Trae 也提供與 Yarah 的第一方整合，讓你可以直接從 Trae 的官方 MCP 市集安裝 Yarah：
    ...
    4. 所需的憑證 API_KEY 與 API_BASE_URL，可以在你的 Yarah **Settings** 中的 **Connect** 分頁下找到
  ...
  <Accordion title="MCP JSON">
    ...
    使用 MCP JSON 將 Yarah MCP 伺服器新增到你的程式碼代理：
...
有些 MCP 用戶端會在設定過程中自動提示你登入，而其他用戶端若自動流程無法完成，則可能需要手動進行身份驗證或專案綁定步驟。無論哪種方式，身份驗證都會開啟一個瀏覽器視窗，你可以在其中登入你的 Yarah 帳戶，並授予 MCP 用戶端組織/專案存取權限。
...
<AccordionGroup>
  <Accordion title="Cursor">
    ...
    在選單 **Preferences → Cursor Settings → Tools & MCP** 中，你應該會看到 Yarah MCP 伺服器已成功新增。
    Yarah MCP 伺服器狀態為「Needs authentication」，你可以點選 `connect` 按鈕進行身份驗證。
  </Accordion>
  ...
  <Accordion title="Claude Code">
    ...
    claude /mcp
    ```
    選擇「yarah」伺服器，然後選擇「Authenticate」以開始身份驗證流程。
  ...
  <Accordion title="Cline">
    你可以在終端機中執行以下指令，將 Yarah MCP 伺服器安裝到你的 **Cline**：
    ```bash
    npx -y @smithery/cli mcp add devel/yarah --client cline
    ```
    或者，你也可以參考官方 [Cline 文件](https://docs.cline.bot/mcp/connecting-to-a-remote-server)，以下列參數新增 Yarah MCP 伺服器：
    - Server Name: `yarah`
  ...
  <Accordion title="Roo Code">
    你可以在終端機中執行以下指令，將 Yarah MCP 伺服器安裝到你的 **Roo Code**：
    ```bash
