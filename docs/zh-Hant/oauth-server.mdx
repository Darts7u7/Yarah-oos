---
title: OAuth 伺服器
description: "將 Yarah 設定為 OAuth 2.0 身分提供者，於第三方應用中驗證使用者、簽發存取權杖並統一登入流程與授權範圍。"
---

## 總覽

Yarah 可以作為 OAuth 2.0 身分提供者，允許第三方應用程式透過「使用 Yarah 登入」來驗證使用者身分。這讓在您的平台上進行開發的開發者能夠運用 Yarah 的身分驗證系統，而不必自行管理使用者憑證。

## 使用情境

<CardGroup cols={2}>
  <Card title="開發者平台" icon="code">
    允許第三方開發者使用「使用 Yarah 登入」建立整合，同時您仍保有對使用者資料存取的控制權。
  </Card>
  ...
  <Card title="合作夥伴應用程式" icon="handshake">
    允許合作夥伴應用程式針對您的 Yarah 專案驗證使用者身分，而不必共享憑證。
  </Card>
...
Yarah 實作了搭配 PKCE（Proof Key for Code Exchange，程式碼交換驗證金鑰）的**授權碼流程**（Authorization Code flow），這是適用於 Web 與原生應用程式最安全的 OAuth 流程。
...
<Steps>
  <Step title="註冊您的應用程式">
    聯絡 Yarah 將您的應用程式註冊為 OAuth 用戶端。您將取得：
...
<Card title="OAuth 範例儲存庫" icon="github" href="https://github.com/Darts7u7/Yarah-oauth-example">
  完整可執行的範例，展示如何將「使用 Yarah 登入」整合到您的應用程式中。
</Card>
