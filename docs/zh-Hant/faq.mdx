...
<AccordionGroup>
  ...
  <Accordion title="Yarah 有 `service_role` key / `YARAH_SERVICE_ROLE_KEY` 嗎？">
    ...
    在 dashboard 的 **Project Settings → General** 裡找 API Key（標著 **API Key** 的那一行，寫明「對專案有完全控制權限，不要暴露在前端」），或者執行 `npx @yarahdev/cli secrets get API_KEY`。
  ...
  <Accordion title="我的專案被暫停了，要怎麼重新啟動？">
    ...
    還是卡住？到我們的 [Discord](https://yarah.dev/community) 提問，回應最快。
  </Accordion>

  <Accordion title="不打開瀏覽器，怎麼幫 CLI 登入驗證？">
    `npx @yarahdev/cli login` 會打開瀏覽器登入。在無介面機器、遠端伺服器或 CI 上，改用 user API key，不需要瀏覽器。
  ...
  <Accordion title="這個助手能幫我解決我自己專案裡的具體問題嗎？">
    基本上不行。這個助手是根據 Yarah 的公開文件來回答的，看不到你的專案：既沒辦法除錯，也讀不到你的資料，更查不了你的設定。凡是跟你自己專案相關的，交給你的編碼 agent 來處理。你的 agent 透過 CLI 或 MCP 連著 Yarah，能讀到你的即時後端、結構、資料和日誌，直接幫你除錯，用白話描述問題就行。想自己拿一份後端健康和報錯報告，跑 `npx @yarahdev/cli diagnose`。參見 [Diagnostics & advisor](/agent-native/diagnostics)。
  </Accordion>
  ...
  <Accordion title="怎麼下線或刪除已部署的站點（site）？">
    目前沒有自助下線已部署 [Site](/core-concepts/sites/overview) 的方式——既沒有 `deployments delete` 命令，dashboard 裡也沒有對應操作。已部署的站點託管在外部，所以即使刪除專案（`npx @yarahdev/cli projects delete --project <id>`）也只會清掉後端資源——資料庫、Storage 和後端分支——而*不會*移除已託管的站點。

    實際上這基本不影響使用。如果你確實需要下線某個已部署的站點，請到 [Discord](https://yarah.dev/community) 聯繫 Yarah 團隊。

    有兩個相關操作，和「下線一個已上線站點」並不是一回事：

    - **取消還在跑的建置：** `npx @yarahdev/cli deployments cancel <id>` 會終止一個進行中的部署；它不會下線一個已經上線的站點。
    - **替換目前上線的內容：** 用 `npx @yarahdev/cli deployments deploy ./frontend` 在同一個站點上重新部署——最新一次 ready 的部署會接管這個 URL。
  </Accordion>
</AccordionGroup>
