...
description: "將 Razorpay 訂單、訂閱、手動 Webhook 與 Webhook 履行整合至 Yarah，於 Postgres 中安全記錄付款事件。"
---
...
Razorpay 沒有相當於 Stripe Billing Portal 的功能。Yarah 提供後端路由，在依據 `payments.razorpay_subscriptions` 的 RLS `UPDATE` 政策檢查呼叫方之後，用於取消、暫停與恢復訂閱。

對於一次性產品，建立 Razorpay Order 時只需提供金額、幣別和 receipt 即可。不過，仍建議為可銷售的產品建立 Razorpay Item，這樣在同步之後，產品目錄就能在 Yarah 與 Razorpay 中可見。請將 Order 視為一次付款嘗試，而非您的產品目錄。

## 設定

在 Dashboard -> Payments -> Settings 中或透過管理 API 設定 `test` 與 `live` 的 Razorpay Key ID 與 Key Secret。若尚不存在 Webhook 簽署密鑰，Yarah 會自動產生一個。
...
首先建立一個由應用程式擁有的待處理訂單（pending order）。接著使用目前的 Yarah 使用者權杖建立一個 Razorpay Order。
