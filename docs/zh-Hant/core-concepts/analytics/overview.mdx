...
`@yarah/cli posthog setup` 將您的 Yarah 專案連結到 PostHog，然後列印官方 [PostHog 嚮導](https://posthog.com/docs/libraries/wizard) 命令 (`npx -y @posthog/wizard@latest`) 供您（或您的代理）接下來執行。嚮導檢測您的框架，安裝正確的 PostHog SDK，並放入初始化程式碼，以便頁面瀏覽、自動擷取事件和工作階段錄製開始流動。
...
<CardGroup cols={2}>
  ...
  <Card title="Yarah CLI" icon="terminal" href="/quickstart">
    `npx @yarah/cli posthog setup` 將您的 Yarah 專案連結到 PostHog，然後列印嚮導命令。
  </Card>
</CardGroup>
