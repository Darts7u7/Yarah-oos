...
description: "Yarah 分析由 PostHog 驅動，追蹤應用事件、使用者留存與漏斗指標，協助團隊做出資料導向的產品決策與優化。"
---

使用 Yarah 分析來瞭解人們如何實際使用您的應用程式：頁面流量、保留和工作階段回放，所有這些都透過將 PostHog 專案連接到您的 Yarah 後端來進行。連接後，儀表板在您的 PostHog 資料上呈現流量、使用者保留和工作階段回放頁面，無需離開 Yarah。
...
<Frame caption="分析儀表板：隨著時間推移的 KPI 加上按頁面、國家和裝置的細分。">
  <img src="/images/dashboard-analytics.png" alt="Yarah 分析儀表板顯示訪客 KPI、訪客趨勢圖表以及頂級頁面、國家和裝置細分" />
</Frame>

<Note>
  PostHog 仍然是事件、儀表板、見解和錄製的真實來源。Yarah 為日常檢查呈現一個集中的子集，然後對於超出其範圍的任何內容都深入連結到 PostHog。
</Note>
...
從儀表板中的分析頁面連接 PostHog。Yarah 為您配置或連結 PostHog 專案，儲存認證到伺服器端，並在連接成功後解鎖流量、保留和工作階段回放頁面。
...
`@yarahdev/cli posthog setup` 將您的 Yarah 專案連結到 PostHog，然後列印官方 [PostHog 嚮導](https://posthog.com/docs/libraries/wizard) 命令 (`npx -y @posthog/wizard@latest`) 供您（或您的代理）接下來執行。嚮導檢測您的框架，安裝正確的 PostHog SDK，並放入初始化程式碼，以便頁面瀏覽、自動擷取事件和工作階段錄製開始流動。
...
分析設定對話（側邊欄中的齒輪圖示）讓管理員審查連結的 PostHog 專案，直接跳轉到 PostHog，並在需要時中斷連接。中斷連接只會中斷 Yarah ↔ PostHog 連結；您的 PostHog 專案、事件和錄製保持不變。
...
<CardGroup cols={2}>
  ...
  <Card title="Yarah CLI" icon="terminal" href="/quickstart">
    `npx @yarahdev/cli posthog setup` 將您的 Yarah 專案連結到 PostHog，然後列印嚮導命令。
  </Card>
</CardGroup>
