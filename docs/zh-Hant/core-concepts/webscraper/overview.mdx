...
在此提示背後，`npx @yarah/cli webscraper apify login` 會取得您由 Yarah 代管的 Apify 權杖，以無頭方式（不需要瀏覽器 OAuth）對本機 Apify CLI 進行身分驗證，並安裝 Apify 代理技能。之後，代理會從 Apify Store 挑選一個 actor、啟動執行，並讀回結果。
...
<CardGroup cols={2}>
  <Card title="Yarah CLI" icon="terminal" href="/quickstart">
    `npx @yarah/cli webscraper apify connect` 會將您的專案連結至 Apify，然後登入您的本機代理。
  </Card>
