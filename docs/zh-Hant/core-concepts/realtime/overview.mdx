...
description: "透過 Yarah 即時頻道推送資料庫變更、廣播訊息、線上狀態（presence）並整合 webhook 分發至下游服務。"
---

當你的應用程式需要在不重新整理頁面的情況下更新時，可以使用 Yarah Realtime。用戶端訂閱像 `order:123` 或 `chat:room-1` 這樣的頻道，然後透過 WebSocket 接收資料庫變更、廣播與線上狀態更新。當另一個服務需要接收事件時，頻道也可以將相同的訊息分發到 webhook URL。
...
當另一個服務需要接收每則訊息時，可以為頻道附加 webhook URL。Yarah 會把事件酬載發佈到每個已設定的 URL，附上包含事件名稱、頻道與訊息 ID 的標頭，重試暫時性的網路失敗，並在訊息歷史中記錄 webhook 傳送次數。
