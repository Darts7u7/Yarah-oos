---
title: Next.js
description: "瞭解如何建立 Yarah 專案，並運用 AI 代理程式編碼快速建構 Next.js 全端應用程式，包含資料庫與身份驗證。"
---

瞭解如何建立 Yarah 專案並使用 AI 工具（如 Cursor）建構 Next.js 應用程式。

## 1. 建立 Yarah 專案

在 [yarah.dev](https://yarah.dev) 建立新的 Yarah 專案。

## 2. 連接 Yarah

兩個部分：

- **CLI 連結** — 查看[快速開始](/quickstart)以執行 `npx @yarahdev/cli link --project-id <your-project-id>` 以便代理可以讀取您的專案 ID 和金鑰。
...
執行開發伺服器，在瀏覽器中訪問 [http://localhost:3000/sports](http://localhost:3000/sports)，您應該看到運動清單。
