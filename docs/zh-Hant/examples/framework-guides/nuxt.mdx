---
title: Nuxt
description: "了解如何建立 Yarah 專案，並運用 AI 代理程式編碼快速建構 Nuxt 全端應用程式，含資料庫、身份驗證與儲存。"
---

import Installation from '/snippets/sdk-installation.mdx';

了解如何建立 Yarah 專案，並使用 Cursor 等 AI 工具建構 Nuxt 應用程式。

## 1. 建立 Yarah 專案

在 [yarah.dev](https://yarah.dev) 建立一個新的 Yarah 專案。

## 2. 連接 Yarah

分為兩部分：

- **CLI 連結** — 請參閱 [快速入門](/quickstart)，執行 `npx @yarahdev/cli link --project-id <your-project-id>`，讓代理程式能夠讀取你的專案 ID 與金鑰。
