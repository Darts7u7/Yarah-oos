---
title: "概覽"
description: "Yarah 是專為 AI 代理程式編碼打造的一體化開源後端平台，內建 Postgres、身份驗證、儲存、函式與 AI 閘道。"
---
...
Yarah 為您的編碼代理提供資料庫、身份驗證、儲存、即時、邊緣函數、網站和模型網關，以端到端地完成全棧應用程式。

## 首先連接

必需的設置。選擇一條路徑以在開始構建之前將您的環境連結到 Yarah。從 CLI 開始進行通用的基於終端的設置；如果您的編輯器支持它，請添加 MCP（Claude Code、Cursor、Windsurf、Codex）。您可以並排執行兩者。

<CardGroup cols={2}>
  <Card title="透過 CLI 連接" icon="terminal" href="/quickstart">
    `npx @yarahdev/cli login` 並連結一個專案。在任何終端中工作，不需要 MCP。
  </Card>
...
在尋找特定工具的說明嗎？瀏覽[代理目錄](https://yarah.dev/agents)以查找針對 Claude Code、Cursor、Windsurf、Codex 等的測試設置步驟。
...
在[更新日誌](https://yarah.dev/changelogs)中跟蹤新功能和修復。
