---
title: 合作夥伴整合
description: "瞭解如何以聯合品牌或白標合作夥伴形式整合 Yarah，為您的客戶提供代理原生後端服務、客製化平台體驗與帳單分潤機制。"
---

## 概觀

Yarah 提供兩種合作夥伴模式，能將我們的後端即服務（Backend-as-a-Service）平台無縫整合到您的應用程式中。無論您是在尋找聯合品牌解決方案，還是完全白標的體驗，我們都有適合您的合作夥伴模式。

## 合作夥伴模式

<CardGroup cols={2}>
  <Card title="聯合品牌合作夥伴關係" icon="handshake">
    非常適合希望將 Yarah 與自身服務一併提供的平台。
    整合流程非常簡單——開發者只需一鍵即可連接到 Yarah 平台，不需要複雜的 OAuth 流程。
  </Card>
...
<Steps>
  <Step title="申請合作夥伴關係">
    透過我們的[合作夥伴信箱](mailto:partnerships@yarah.dev)提交您的合作夥伴申請。
...
### 聯合品牌功能
在聯合品牌模式下，開發者明確知道自己是 Yarah 平台的使用者（透過相同的電子郵件位址關聯）。登入 Yarah 平台後，開發者對透過合作夥伴建立的專案擁有完整的管理權限，並需要依照 Yarah 的計費方案付費。

合作夥伴平台可以：
- 將使用者帳號（姓名、電子郵件）與 Yarah 同步
- 將專案同步至 Yarah
- 查詢專案連接資訊，以運用已完成的後端能力。

### 白標功能
在白標模式下，開發者不知道 Yarah 的存在，也無法在 Yarah 平台上看到合作夥伴建立的專案。
...
<Tabs>
  <Tab title="聯合品牌 API">
    ...
    **回應：**
    ...
    #### 處理專案數量限制
    注意：由於 Yarah 免費方案有專案數量限制，來自合作夥伴平台的專案同步作業可能會失敗。在這種情況下，回應內容將為：
...
### 專案地區數值
Yarah 提供全球化的部署能力。每個專案都可以部署到不同的地區：
- `us-east` - 美國東岸
- `us-west` - 美國西岸
- `ap-southeast` - 亞太東南
- `eu-central` - 歐洲中部
...
- 與我們的團隊安排一次[技術評估會議](https://calendly.com/tony-chang-yarah/45min)
