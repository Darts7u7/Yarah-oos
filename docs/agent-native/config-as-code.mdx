...
description: Manage Yarah auth, SMTP, storage, and deployment settings from a single yarah.toml using the CLI's plan, apply, and export workflow.
---

## Overview

`yarah.toml` is a declarative, version-controlled snapshot of a subset of your project's config: auth policy, allowed redirect URLs, password rules, SMTP, storage upload size, realtime and schedule retention, and the deployment subdomain. The CLI provides three commands that work against this file:

- **`config plan`**: diff `yarah.toml` against the linked project. Shows what would change.
- **`config apply`**: push the diff to the live project. Per-change capability gating, env-resolved secrets, dry-run mode.
- **`config export`**: pull current project state and write a fresh `yarah.toml`. Useful for bootstrapping from an existing project.

You keep **one** `yarah.toml` in your repo. To apply it to a different environment (staging, prod, a teammate's local backend, or a self-hosted instance), point the CLI at a different project (re-link or use `--project-id`). Secrets are read from environment variables via `env(...)` references, so the file itself stays free of credentials and can be committed safely.

This works the same way on Yarah Cloud projects and on self-hosted OSS deployments.

<Note>
  All examples use `npx @yarah/cli`. Do not install the CLI globally.
</Note>
...
| Command | Purpose |
|---------|---------|
| `config plan` | Show diff between `yarah.toml` and live project state |
| `config apply` | Apply `yarah.toml` to the live project |
...
| `config export` | Pull live config and write `yarah.toml` |
| `config export --force` | Overwrite an existing `yarah.toml` without confirmation |

`config plan` and `config apply` read `yarah.toml`; pass `--file <path>` if it lives somewhere other than `./yarah.toml`. `config export` writes the file; pass `--out <path>` to write it to a custom location.
...
<Steps>
  <Step title="Bootstrap from an existing project">
    ...
    ```bash
    npx @yarah/cli config export
    ```

    This writes `yarah.toml` reflecting the current backend state.
  </Step>

  <Step title="Commit it">
    Check `yarah.toml` into version control. Secrets are referenced via `env(...)`, so the file is safe to commit.
  </Step>
  ...
  <Step title="Edit and plan">
    ...
    ```bash
    npx @yarah/cli config plan
    ```
  </Step>

  <Step title="Apply">
    ```bash
    npx @yarah/cli config apply
    ```
  ...
  <Step title="Apply the same file elsewhere">
    ...
    ```bash
    npx @yarah/cli --project-id <staging-project-id> config apply
    ```
...
```bash
npx @yarah/cli config plan
npx @yarah/cli config plan --file ./config/yarah.toml
npx @yarah/cli --json config plan
```

`plan` reads `yarah.toml`, fetches live state via `/api/metadata` and the optional config endpoints for storage, realtime, and schedules, then prints a rendered diff. It also tags any section the live backend doesn't expose yet (older self-hosted versions, etc.). Apply will skip those instead of failing the whole run.
...
```bash
npx @yarah/cli config apply
npx @yarah/cli config apply --dry-run
npx @yarah/cli config apply --auto-approve
npx @yarah/cli --json config apply --auto-approve
```
...
- `--file <path>` overrides the default `./yarah.toml` location.

## `config export`

```bash
npx @yarah/cli config export
npx @yarah/cli config export --out ./config/yarah.toml
npx @yarah/cli config export --force
```

`export` pulls the live project's configurable surface and writes it to a TOML file. Use it to:

- Bootstrap an `yarah.toml` from a project you've been configuring through the dashboard.
...
This is what lets one `yarah.toml` apply cleanly to multiple environments: the dev and prod backends differ only in *which* `SENDGRID_API_KEY` is in scope when you run `apply`.
...
- **Disaster recovery.** A committed `yarah.toml` is a known-good config snapshot. Re-apply it after restoring a project to bring auth, SMTP, storage, retention, and deployment settings back to the expected shape in seconds.
- **Self-hosted and local OSS development.** Run `npx @yarah/cli link --api-base-url http://localhost:7130 --api-key <local-key>` against a docker-compose stack, then point the CLI's config commands at your local OSS instance the same way you'd point at cloud.
