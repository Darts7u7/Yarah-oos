...
<Note>
  All examples use `npx @yarahdev/cli`. Do not install the CLI globally.
</Note>
...
<Steps>
  ...
  <Step title="Apply the same file elsewhere">
    ...
    ```bash
    npx @yarahdev/cli --project-id <staging-project-id> config apply
    ```
...
```bash
npx @yarahdev/cli config plan
npx @yarahdev/cli config plan --file ./config/yarah.toml
npx @yarahdev/cli --json config plan
```
...
```bash
npx @yarahdev/cli config apply
npx @yarahdev/cli config apply --dry-run
npx @yarahdev/cli config apply --auto-approve
npx @yarahdev/cli --json config apply --auto-approve
```
...
```bash
npx @yarahdev/cli config export
npx @yarahdev/cli config export --out ./config/yarah.toml
npx @yarahdev/cli config export --force
```
...
- **Self-hosted and local OSS development.** Run `npx @yarahdev/cli link --api-base-url http://localhost:7130 --api-key <local-key>` against a docker-compose stack, then point the CLI's config commands at your local OSS instance the same way you'd point at cloud.
