...
Run `npx @yarah/cli help <command>` for the flags on any of these.
...
```bash
npx @yarah/cli secrets list          # every key, values hidden
npx @yarah/cli secrets get ANON_KEY  # decrypted value of one key
```
