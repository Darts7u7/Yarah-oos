...
Run `npx @yarahdev/cli help <command>` for the flags on any of these.
...
| `JWT_SECRET` | Secret for HS256-signed JWTs that Yarah accepts, seeded from the deployment's `JWT_SECRET` env var |
| `YARAH_BASE_URL` | The project's API base URL |

```bash
npx @yarahdev/cli secrets list          # every key, values hidden
npx @yarahdev/cli secrets get ANON_KEY  # decrypted value of one key
```
...
`projects update-version` moves the project to the latest Yarah backend version. It targets the linked project by default; pass `--project <id>` to pick another. The command restarts the instance, so expect a brief downtime. Add `--wait` to block until the update finishes.
