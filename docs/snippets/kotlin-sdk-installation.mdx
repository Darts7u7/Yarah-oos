1. Add Yarah dependencies to your project
...
<Tabs>
  <Tab title="Maven Central">
    ...
    dependencies {
        implementation("dev.yarah:yarah-kotlin:0.1.6")
    }
    ```
  </Tab>
    ...
    <AccordionGroup>
      <Accordion title="Option A: Environment Variables">
        ...
        repositories {
            mavenCentral()
            maven {
                url = uri("https://maven.pkg.github.com/Yarah/yarah-kotlin")
                credentials {
...
2. Initialize Yarah SDK

```kotlin
import dev.yarah.createYarahClient
import dev.yarah.auth.Auth
import dev.yarah.database.Database
import dev.yarah.storage.Storage
import dev.yarah.functions.Functions
import dev.yarah.realtime.Realtime
import dev.yarah.ai.AI

val client = createYarahClient(
...
```kotlin
import dev.yarah.YarahLogLevel
...
2. Initialize Yarah SDK (With Chrome Custom Tabs and Session Storage)
...
import dev.yarah.ai.AI
import dev.yarah.auth.Auth
import dev.yarah.auth.BrowserLauncher
import dev.yarah.auth.ClientType
import dev.yarah.auth.SessionStorage
import dev.yarah.database.Database
import dev.yarah.functions.Functions
import dev.yarah.realtime.Realtime
import dev.yarah.storage.Storage

class YarahManager(private val context: Context) {
...
val Context.authDataStore: DataStore<Preferences> by preferencesDataStore(name = "yarah_auth")
...
// Then use it in your Yarah client
install(Auth) {
    browserLauncher = ...
    persistSession = true
