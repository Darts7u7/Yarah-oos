...
description: Sign up, sign in, manage sessions, and update user profiles from Android and JVM apps with the Yarah Kotlin SDK auth client and JWT tokens.
---
...
- `redirectUri` (`String`) - Callback URL where Yarah will redirect after authentication
...
When creating the Yarah client, configure the `browserLauncher` to handle opening URLs:

```kotlin
val client = createYarahClient(baseURL, anonKey) {
    install(Auth) {
...
class AuthCallbackActivity : AppCompatActivity() {
    // Example: read the shared Yarah client from your Application class.
    private val client by lazy { (application as MyApp).yarahClient }
...
`client` should come from the same shared Yarah client instance you use elsewhere in the app, such as an `Application` singleton or your DI container.
...
```kotlin
import dev.yarah.exceptions.YarahHttpException
import dev.yarah.exceptions.YarahException
