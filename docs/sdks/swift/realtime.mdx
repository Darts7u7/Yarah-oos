...
description: Connect over Socket.IO, subscribe to channels, publish events, track presence, and handle reconnects with the Yarah Swift realtime client on Apple.
---
...
The Swift SDK connects to Yarah Realtime over Socket.IO. Subscribe to channel names such as `order:123` or `chat:room-1`, then listen for event names published to those channels.
