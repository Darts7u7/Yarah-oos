---
title: Swift authentication SDK reference
description: Sign up, sign in, verify email, manage sessions, and configure OAuth URL schemes for iOS, macOS, tvOS, and watchOS with the Yarah Swift SDK.
---
...
func handleSignUp(email: String, password: String, name: String?) async {
    do {
        let result = try await yarah.auth.signUp(
            email: email,
            password: password,
            name: name
        )
...
func handleVerifyEmail(email: String, code: String) async {
    do {
        try await yarah.auth.verifyEmail(email: email, code: code)
...
func handleResendCode(email: String) async {
    do {
        try await yarah.auth.resendVerificationEmail(email: email)
...
For users who register with email, the Yarah backend provides three options:
...
3. **Code-based verification** - The Yarah backend sends a 6-digit verification code to the user's email. The client app needs to display a verification screen where users can enter the code, then call `verifyEmail(email:code:)` to complete verification. Only after this can users sign in with email + password.
...
```swift
do {
    let result = try await yarah.auth.signIn(
...
```swift
do {
    if let user = try await yarah.auth.getCurrentUser() {
...
```swift
do {
    let result = try await yarah.auth.setProfile([
        "name": "JohnDev",
        "bio": "iOS Developer",
...
Yarah supports two password reset methods, configured in the backend:
...
```swift
do {
    try await yarah.auth.sendPasswordReset(email: "user@example.com")
...
```swift
do {
    let response = try await yarah.auth.exchangeResetPasswordToken(
        email: "user@example.com",
        code: "123456"
    )
...
```swift
do {
    try await yarah.auth.resetPassword(
        otp: resetToken,
...
- `redirectTo` (`String`) - Callback URL where Yarah will redirect after authentication
...
2. SDK fetches the OAuth authorization URL from Yarah
...
} catch let error as YarahAuthError {
    switch error {
