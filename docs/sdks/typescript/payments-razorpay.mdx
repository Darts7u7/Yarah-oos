...
description: Create Razorpay Orders, verify signatures, and manage Subscriptions from your TypeScript frontend with the Yarah SDK payments.razorpay helpers.
---
...
The SDK calls Razorpay runtime routes with the current Yarah user token. Do not call these routes with provider secret keys.
...
Create an app-owned pending order first. Then create the Razorpay Order through Yarah:

Razorpay Orders can be amount-only, but creating Razorpay Items for one-time sellable products is a good practice because synced Items make the catalog visible in Yarah and Razorpay. Treat Orders as payment attempts.
...
Create or sync a Razorpay Plan first. Then create the subscription through Yarah:
