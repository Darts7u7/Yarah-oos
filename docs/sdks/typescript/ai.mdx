---
title: Model Gateway
description: Build AI features in TypeScript with the OpenAI SDK and the OpenRouter API key provisioned by the Yarah Model Gateway for chat, streaming, and embeddings.
---

## Overview

Yarah provisions an OpenRouter API key for Model Gateway projects. For new AI features, use that key directly with OpenRouter's OpenAI-compatible API instead of the deprecated Yarah AI proxy methods.

Use the Yarah dashboard to copy the active OpenRouter key, store it in `OPENROUTER_API_KEY`, and call OpenRouter from trusted server-side code.
...
## Using Yarah Data with AI

Use the Yarah SDK for database, auth, storage, functions, and realtime. Use OpenRouter directly for model calls.
...
## Legacy Yarah AI Proxy

The previous Yarah AI SDK methods are deprecated:

- `yarah.ai.chat.completions.create()`
- `yarah.ai.images.generate()`
- `yarah.ai.embeddings.create()`

They map to backend proxy routes that still exist for compatibility, but new integrations should use OpenRouter directly. The direct path gives access to OpenRouter's complete and evolving API surface without waiting for Yarah SDK wrapper updates.

## Model Catalog

Use the Yarah dashboard or OpenRouter model catalog to choose a model. Model IDs use the `provider/model` format, such as `openai/gpt-4o`, `anthropic/claude-3.5-haiku`, and `google/gemini-2.5-flash-image`.
