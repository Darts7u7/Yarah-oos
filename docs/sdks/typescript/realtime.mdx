---
title: TypeScript realtime SDK reference
description: Subscribe to channels, publish events, track presence, and handle reconnects and token refreshes over Socket.IO with the Yarah TypeScript SDK.
---
...
The TypeScript SDK opens one Socket.IO connection to your Yarah backend. You subscribe to named channels, listen for event names, and optionally publish events back to channels you have joined.

Events can come from two places:

- Database triggers that call `realtime.publish(channel, event, payload)`.
- Clients that call `yarah.realtime.publish(channel, event, payload)`.
