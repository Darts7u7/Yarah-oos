...
description: Sign up, sign in, manage sessions, and update user profiles from web and Node.js apps with the Yarah TypeScript SDK auth client and JWT tokens.
---
...
<Note>
  When `requireEmailVerification` is true, `accessToken` will be null until the user verifies their email. Yarah sends a verification email with either a link or a 6-digit code, based on your dashboard configuration (`verifyEmailMethod`). For code verification, implement a page that prompts the user to enter the code (see [verifyEmail()](#verifyemail)). For link verification, provide a `redirectTo` URL that should receive the browser after Yarah verifies the token. Recommended: use your sign-in page as `redirectTo`, then show a success message and ask the user to sign in with their email and password.
</Note>
...
<Note>
  After OAuth redirect, SDK automatically detects the callback `yarah_code`, exchanges it for a
...
<Note>
  ...
  or `scope`; Yarah sets those values on the server and ignores colliding client-provided keys.
</Note>

<Note>
  Custom providers must be configured first in the Yarah dashboard under `Auth Methods` with client credentials and an OIDC discovery URL.
</Note>
...
```javascript
const { data, error } = await yarah.auth.getProfile('usr_xyz789');
...
```javascript
const { data, error } = await yarah.auth.setProfile({
  name: 'JohnDev',
  bio: 'Full-stack developer',
  avatar_url: 'https://example.com/avatar.jpg',
...
Resend email verification when the previous OTP has expired or was not received. Uses the method configured in auth settings (`verifyEmailMethod`). When method is `code`, sends a 6-digit numeric code. When method is `link`, sends a browser verification link that goes through an Yarah backend endpoint first.
...
```javascript
const { data, error } = await yarah.auth.resendVerificationEmail({
  email: 'user@example.com',
  redirectTo: 'http://localhost:3000/sign-in',
});
...
Send password reset email using the method configured in auth settings (`resetPasswordMethod`). When method is `code`, sends a 6-digit numeric code for two-step flow. When method is `link`, sends a browser reset link that goes through an Yarah backend endpoint first.
...
```javascript
const { data, error } = await yarah.auth.sendResetPasswordEmail({
  email: 'user@example.com',
...
```javascript
const { data, error } = await yarah.auth.exchangeResetPasswordToken({
  email: 'user@example.com',
  code: '123456',
});
...
- `token`: Present only when `yarah_status=ready`. Pass this value to `resetPassword({ otp })`.
...
Only render the reset-password form when `yarah_status=ready` and `token` is present.
...
const { data, error } = await yarah.auth.resetPassword({
  newPassword: 'newSecurePassword123',
