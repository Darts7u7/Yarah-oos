...
description: Create Stripe Checkout sessions, open the Billing Portal, and manage subscriptions from your TypeScript frontend with the Yarah SDK payments.stripe helpers.
---
...
The SDK methods require the current Yarah user token. Anonymous Yarah tokens can be used for guest one-time checkout, but an Yarah API key is not a substitute because the backend needs user context for local session rows.
...
If one-time checkout includes a `subject` and there is no existing Stripe customer mapping yet, Yarah lets Stripe create the customer during Checkout and backfills the subject mapping from the completion webhook.
...
The SDK methods call runtime routes using the current Yarah token. The backend inserts local session rows using the caller context:
...
<Warning>
  ...
  variables. Configure Stripe keys through the Yarah dashboard or CLI.
</Warning>
