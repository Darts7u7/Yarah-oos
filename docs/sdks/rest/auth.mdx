...
| `redirectTo` | string | No | Used for link-based email verification. The email link always opens an Yarah backend endpoint first; after the token is verified, Yarah redirects the browser to this URL with the verification result. Required when `verifyEmailMethod` is `link`. This URL must be included in `allowedRedirectUrls`. Recommended: use your app's sign-in page. |

### Example (Web Client)

```bash
curl -X POST "https://your-app.apps.yarah.dev/api/auth/users" \
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/auth/users?client_type=mobile" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
    "password": "securepassword123",
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/auth/sessions" \
  -H "Content-Type: application/json" \
  -d '{
    "method": "password",
    "email": "user@example.com",
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/auth/sessions?client_type=mobile" \
  -H "Content-Type: application/json" \
  -d '{
    "method": "password",
    "email": "user@example.com",
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/auth/email/send-otp" \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/auth/sessions?client_type=server" \
  -H "Content-Type: application/json" \
  -d '{
    "method": "otp",
    "email": "user@example.com",
...
- Yarah does not create a user when the code is sent. If the email is new, a verified passwordless user is created only after successful verification.
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/auth/refresh" \
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/auth/refresh?client_type=mobile" \
  -H "Content-Type: application/json" \
  -d '{
    "refreshToken": "eyJhbGciOiJIUzI1NiIs..."
  }'
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/auth/logout"
```
...
```bash
curl "https://your-app.apps.yarah.dev/api/auth/sessions/current" \
...
```bash
curl -X PATCH "https://your-app.apps.yarah.dev/api/auth/profiles/current" \
  -H "Authorization: Bearer your-jwt-token" \
  -H "Content-Type: application/json" \
  -d '{
    "profile": {
...
```bash
curl "https://your-app.apps.yarah.dev/api/auth/profiles/123e4567-e89b-12d3-a456-426614174000"
```
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/auth/email/send-verification" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
    "redirectTo": "http://localhost:3000/sign-in"
...
- Success: `?yarah_status=success&yarah_type=verify_email`
- Error: `?yarah_status=error&yarah_type=verify_email&yarah_error=...`
- `yarah_status`: Result of the browser link flow. For verification, values are `success` or `error`.
- `yarah_type`: Flow identifier. For verification links this is always `verify_email`.
- `yarah_error`: Present only when `yarah_status=error`. Human-readable error message for display or logging.
- Recommended handling: use your sign-in page as `redirectTo`. When `yarah_status=success`, show a confirmation message and ask the user to sign in with their email and password.
- If `redirectTo` is not allowlisted, Yarah returns a `400` error whose message includes the rejected URL and whose `nextActions` tells you to add it to `allowedRedirectUrls`.

### Example (Web Client)

```bash
curl -X POST "https://your-app.apps.yarah.dev/api/auth/email/verify" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
    "otp": "123456"
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/auth/email/verify?client_type=mobile" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
    "otp": "123456"
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/auth/email/exchange-reset-password-token" \
  -H "Content-Type: application/json" \
  -d '{
    "email": "user@example.com",
    "code": "123456"
...
- Ready to reset: `?token=...&yarah_status=ready&yarah_type=reset_password`
- Error: `?yarah_status=error&yarah_type=reset_password&yarah_error=...`
- `token`: Present only when `yarah_status=ready`. Pass this value to `POST /api/auth/email/reset-password` as `otp`.
- `yarah_status`: Result of the browser link flow. For reset links, values are `ready` or `error`.
- `yarah_type`: Flow identifier. For reset links this is always `reset_password`.
- `yarah_error`: Present only when `yarah_status=error`. Human-readable error message for display or logging.
- Your app should only render the reset-password form when `yarah_status=ready` and `token` is present.
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/auth/email/reset-password" \
  -H "Content-Type: application/json" \
  -d '{
...
returned identity token. Yarah hashes it before comparing it with Apple's signed `nonce`
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/auth/id-token?client_type=mobile" \
  -H "Content-Type: application/json" \
  -d '{
    "provider": "apple",
...
Yarah verifies Apple's signature, issuer, expiry, subject, nonce, and token audience. The
...
```bash
curl -X PUT "https://your-app.apps.yarah.dev/api/auth/oauth/apple/config" \
...
Apple private-relay addresses are supported. Yarah finds returning accounts by Apple's stable
...
identity token; Yarah never creates a fabricated placeholder address.
...
|-------|------|-------------|
| `user` | object | Authenticated Yarah user, including `id`, `email`, verification state, providers, profile, metadata, and timestamps. |
| `accessToken` | string | Yarah access token for authenticated API requests. |
...
```json
{
  ...
  "accessToken": "YARAH_ACCESS_TOKEN",
  "refreshToken": "YARAH_REFRESH_TOKEN"
}
```
...
<Note>
  ...
  `response_type`, or `scope`; Yarah/provider-generated values win and colliding client
  values are ignored.
</Note>
...
curl "https://your-app.apps.yarah.dev/api/auth/oauth/google?redirect_uri=https://myapp.com/callback&code_challenge=$CODE_CHALLENGE&prompt=select_account"
```

```bash
# Custom provider example
curl "https://your-app.apps.yarah.dev/api/auth/oauth/custom/acme?redirect_uri=https://myapp.com/callback&code_challenge=$CODE_CHALLENGE"
```
...
```
https://myapp.com/callback?yarah_code=abc123...
```

<Note>
The `yarah_code` is a temporary authorization code that must be exchanged for tokens using the `/api/auth/oauth/exchange` endpoint.
</Note>
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `code` | string | Yes | The `yarah_code` received in the callback |
| `code_verifier` | string | Yes | The original code_verifier used to generate the code_challenge |

### Example

```bash
curl -X POST "https://your-app.apps.yarah.dev/api/auth/oauth/exchange?client_type=mobile" \
  -H "Content-Type: application/json" \
  -d '{
    "code": "abc123...",
...
let authURL = "https://your-app.apps.yarah.dev/api/auth/oauth/google" +
...
// 4. Handle callback with yarah_code
// myapp://callback?yarah_code=abc123...
...
```bash
curl "https://your-app.apps.yarah.dev/api/auth/public-config"
```
...
```bash
curl -X DELETE "https://your-app.apps.yarah.dev/api/auth/users" \
...
```bash
curl "https://your-app.apps.yarah.dev/api/auth/config" \
  -H "Authorization: Bearer admin-jwt-token"
```
...
- If `allowedRedirectUrls` is empty, Yarah allows all redirects for developer convenience. This is insecure for production and should be avoided outside local development.
...
```bash
curl -X PUT "https://your-app.apps.yarah.dev/api/auth/config" \
  -H "Authorization: Bearer admin-jwt-token" \
  -H "Content-Type: application/json" \
  -d '{
    "requireEmailVerification": true,
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `code` | string | Yes | Authorization code or JWT from Yarah Cloud |

#### Example

```bash
curl -X POST "https://your-app.apps.yarah.dev/api/auth/admin/sessions/exchange" \
  -H "Content-Type: application/json" \
...
`yarah_admin_refresh_token` httpOnly cookie and does not share the app/user
refresh cookie.
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/auth/admin/refresh" \
  -H "X-CSRF-Token: abc123..." \
  --cookie "yarah_admin_refresh_token=..."
```
