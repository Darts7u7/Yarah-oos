...
description: Upload, download, list, and manage files in Yarah storage buckets over the REST API, backed by local disk or S3-compatible object storage.
---
...
Yarah supports two types of storage backends:
...
- **Local Storage**: Direct upload to Yarah API
...
|-------|------|----------|-------------|
| `filename` | string | Yes | Object key to upload to (used verbatim) |
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/storage/buckets/avatars/upload-strategy" \
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/storage/buckets/avatars/objects/profile-photo.jpg/confirm-upload" \
  -H "Authorization: Bearer your-jwt-token" \
  -H "Content-Type: application/json" \
  -d '{
    "size": 102400,
...
```bash
curl -X PUT "https://your-app.apps.yarah.dev/api/storage/buckets/avatars/objects/users/profile.jpg" \
  -H "Authorization: Bearer your-jwt-token" \
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/storage/buckets/uploads/objects" \
  -H "Authorization: Bearer your-jwt-token" \
...
- **Local Storage**: Direct download from Yarah API
...
```bash
curl "https://your-app.apps.yarah.dev/api/storage/buckets/avatars/download-strategy/objects/profile.jpg" \
  -H "Authorization: Bearer your-jwt-token"
```
...
```bash
curl "https://your-app.apps.yarah.dev/api/storage/buckets/avatars/objects/users/profile.jpg" \
  -H "Authorization: Bearer your-jwt-token" \
  -o profile.jpg
```
...
```bash
curl -X DELETE "https://your-app.apps.yarah.dev/api/storage/buckets/avatars/objects/users/profile.jpg" \
  -H "Authorization: Bearer your-jwt-token"
```
...
```bash
# Filter by prefix
curl "https://your-app.apps.yarah.dev/api/storage/buckets/avatars/objects?prefix=users/&limit=50" \
  -H "Authorization: Bearer your-jwt-token"

# Search by key
curl "https://your-app.apps.yarah.dev/api/storage/buckets/avatars/objects?search=profile" \
  -H "Authorization: Bearer your-jwt-token"
```
...
```bash
curl "https://your-app.apps.yarah.dev/api/storage/buckets" \
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/storage/buckets" \
  -H "Authorization: Bearer admin-jwt-token" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X PATCH "https://your-app.apps.yarah.dev/api/storage/buckets/user-uploads" \
  -H "Authorization: Bearer admin-jwt-token" \
  -H "Content-Type: application/json" \
  -d '{"isPublic": true}'
```
...
```bash
curl -X DELETE "https://your-app.apps.yarah.dev/api/storage/buckets/old-uploads" \
  -H "Authorization: Bearer admin-jwt-token"
```
