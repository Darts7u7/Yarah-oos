...
description: Query, insert, update, and delete rows in your Yarah Postgres tables over HTTP using PostgREST-style operations with JWT tokens and Row Level Security.
---
...
```bash
# Get all posts
curl "https://your-app.apps.yarah.dev/api/database/records/posts" \
  -H "Authorization: Bearer your-jwt-token"

# Get posts with filters
curl "https://your-app.apps.yarah.dev/api/database/records/posts?status=eq.published&order=createdAt.desc&limit=10" \
  -H "Authorization: Bearer your-jwt-token"

# Select specific columns
curl "https://your-app.apps.yarah.dev/api/database/records/posts?select=id,title,author" \
  -H "Authorization: Bearer your-jwt-token"
```
...
[
  {
    ...
    "title": "Getting Started with Yarah",
...
curl -X POST "https://your-app.apps.yarah.dev/api/database/records/posts" \
  -H "Authorization: Bearer your-jwt-token" \
...
curl -X PATCH "https://your-app.apps.yarah.dev/api/database/records/posts?id=eq.248373e1-0aea-45ce-8844-5ef259203749" \
  -H "Authorization: Bearer your-jwt-token" \
  -H "Content-Type: application/json" \
  -H "Prefer: return=representation" \
  -d '{
...
curl -X PATCH "https://your-app.apps.yarah.dev/api/database/records/posts?status=eq.draft" \
  -H "Authorization: Bearer your-jwt-token" \
  -H "Content-Type: application/json" \
  -d '{"status": "archived"}'
```
...
```bash
# Delete by ID
curl -X DELETE "https://your-app.apps.yarah.dev/api/database/records/posts?id=eq.248373e1-0aea-45ce-8844-5ef259203749" \
  -H "Authorization: Bearer your-jwt-token"

# Delete with Prefer header to see deleted records
curl -X DELETE "https://your-app.apps.yarah.dev/api/database/records/posts?status=eq.archived" \
  -H "Authorization: Bearer your-jwt-token" \
  -H "Prefer: return=representation"
```
...
curl "https://your-app.apps.yarah.dev/api/database/records/posts?limit=20&offset=0"

# Page 2 (next 20 records)
curl "https://your-app.apps.yarah.dev/api/database/records/posts?limit=20&offset=20"
```
...
```bash
# Multiple conditions
curl "https://your-app.apps.yarah.dev/api/database/records/posts?status=eq.published&author_id=eq.123&order=createdAt.desc"

# Search with pattern
curl "https://your-app.apps.yarah.dev/api/database/records/users?email=ilike.*@company.com"

# In list
curl "https://your-app.apps.yarah.dev/api/database/records/orders?status=in.(pending,processing)"

# Null check
curl "https://your-app.apps.yarah.dev/api/database/records/tasks?completed_at=is.null"
```
...
curl -X POST "https://your-app.apps.yarah.dev/api/database/records/user_settings" \
  -H "Authorization: Bearer your-jwt-token" \
  -H "Content-Type: application/json" \
...
curl -X POST "https://your-app.apps.yarah.dev/api/database/rpc/get_user_stats" \
  -H "Authorization: Bearer your-jwt-token" \
  -H "Content-Type: application/json" \
  -d '{"user_id": "123e4567-e89b-12d3-a456-426614174000"}'

# Call a function without parameters
curl -X POST "https://your-app.apps.yarah.dev/api/database/rpc/get_total_count" \
  -H "Authorization: Bearer your-jwt-token"
```
...
```bash
curl "https://your-app.apps.yarah.dev/api/database/functions" \
...
```bash
curl "https://your-app.apps.yarah.dev/api/database/indexes" \
  -H "Authorization: Bearer admin-jwt-token-Or-API-Key"
```
...
```bash
curl "https://your-app.apps.yarah.dev/api/database/policies" \
  -H "Authorization: Bearer admin-jwt-token-Or-API-Key"
```
...
```bash
curl "https://your-app.apps.yarah.dev/api/database/triggers" \
  -H "Authorization: Bearer admin-jwt-token-Or-API-Key"
```
...
Get the history of successful custom migrations. Yarah stores these in `system.custom_migrations`, but the endpoint returns only the migration metadata and executed statements. **Requires admin authentication.**
...
```bash
curl "https://your-app.apps.yarah.dev/api/database/migrations" \
  -H "Authorization: Bearer admin-jwt-token-Or-API-Key"
```
...
|-------|------|----------|-------------|
| `version` | string | Yes | Numeric migration version (up to 64 digits). Accepts Drizzle-style sequential prefixes (e.g. `0001`) or a `YYYYMMDDHHmmss` timestamp. Versions are compared numerically. The Yarah CLI generates 14-digit UTC timestamps for local migration filenames. |
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/database/migrations" \
  -H "Authorization: Bearer admin-jwt-token-Or-API-Key" \
  -H "Content-Type: application/json" \
  -d '{
    "version": "20260416170500",
...
<Warning>
Do not include `BEGIN`, `COMMIT`, or `ROLLBACK` inside a custom migration. Yarah executes the migration inside its own transaction.
</Warning>
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/database/advance/rawsql" \
  -H "Authorization: Bearer admin-jwt-token-Or-API-Key" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/database/advance/rawsql/unrestricted" \
  -H "Authorization: Bearer admin-jwt-token-Or-API-Key" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/database/advance/export" \
  -H "Authorization: Bearer admin-jwt-token-Or-API-Key" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/database/advance/import" \
  -H "Authorization: Bearer admin-jwt-token-Or-API-Key" \
  -F "file=@backup.sql" \
  -F "truncate=false"
```
...
```bash
# Import from CSV
curl -X POST "https://your-app.apps.yarah.dev/api/database/advance/bulk-upsert" \
  -H "Authorization: Bearer admin-jwt-token-Or-API-Key" \
  -F "file=@data.csv" \
  -F "table=posts" \
  -F "upsertKey=id"
