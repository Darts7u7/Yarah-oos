...
description: Call OpenRouter over HTTP with the key provisioned by Yarah, plus reference the legacy Model Gateway proxy endpoints for chat, images, and embeddings.
---

## Overview

New AI integrations should call OpenRouter directly with the OpenRouter key provisioned by Yarah. The Yarah AI proxy routes for chat, image generation, and embeddings are deprecated compatibility endpoints.
...
Use either an admin JWT bearer token or an Yarah API key bearer token in the `Authorization` header.
...
This route validates an Yarah-shaped request, forwards it to OpenRouter, and returns an Yarah-shaped response. It does not expose the complete OpenRouter API surface.
...
This route forwards an Yarah-shaped image request to OpenRouter chat completions with image output. It is retained for compatibility only.
...
This route forwards an Yarah-shaped embeddings request to OpenRouter. Prefer the direct OpenRouter embeddings endpoint for new code.
