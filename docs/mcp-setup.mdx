---
title: "Set up MCP for Cursor, Claude, Windsurf, and more"
sidebarTitle: "MCP setup"
description: "Connect Cursor, Claude, Windsurf, and other editors to Yarah over MCP so AI agents can query your database, auth, storage, and functions."
---

## Overview

The Yarah MCP server gives your AI coding assistant direct access to your backend: database queries, schema management, storage, and more.

Follow the instructions below for your AI client. Don't see your tool? Browse the [agent directory](https://yarah.dev/agents) for the full list of tested integrations.

## What the MCP server can do

Once connected, the Yarah MCP server exposes these tools to your AI assistant. All run against your own project; write and admin actions require your admin API key.
...
- An **Yarah project** - [create one here](https://yarah.dev) if you haven't already
...
<AccordionGroup>
  <Accordion title="Cursor">
    ...
    Alternatively, you can install the Yarah MCP server manually:
    ...
    4. Paste the MCP JSON from your Yarah project
    5. Save the configuration file
    6. You should see a green dot, indicating Yarah is installed successfully
    ...
    If the connection is successful, you will see the **Connected** status indicator on your Yarah dashboard (top right corner).
  ...
  <Accordion title="Claude Code">
    ...
    Add the Yarah MCP server to your **Claude Code**:
    ...
    If the connection is successful, you will see the **MCP Connected** status indicator on your Yarah dashboard (top right corner).
  ...
  <Accordion title="GitHub Copilot">
    ...
    Add the Yarah MCP server to your **GitHub Copilot**:
  ...
  <Accordion title="Google Antigravity">
    ...
    Add the Yarah MCP server to your **Antigravity**:
  ...
  <Accordion title="Kiro">
    ...
    Add the Yarah MCP server to your **Kiro**:
  ...
  <Accordion title="Codex">
    ...
    Add the Yarah MCP server to your **Codex**:
  ...
  <Accordion title="Cline">
    ...
    Add the Yarah MCP server to your **Cline**:
  ...
  <Accordion title="Windsurf">
    ...
    Add the Yarah MCP server to your **Windsurf**:
  ...
  <Accordion title="Trae">
    ...
    Add the Yarah MCP server to your **Trae**:
    ...
    Alternatively, Trae offers a first-party integration with Yarah, allowing you to install Yarah directly from Trae's official MCP marketplace:

    1. Open Trae's **Settings** → navigate to **MCP**
    2. Click on **Add** and then select **Add from Marketplace**
    3. Search "Yarah" and add it from the marketplace

    ![Trae Marketplace](/images/mcp-setup/trae-3.png)

    4. For required credentials API_KEY and API_BASE_URL, you can find them in your Yarah **Settings**, under the **Connect** tab.
  ...
  <Accordion title="Qoder">
    ...
    Add the Yarah MCP server to your **Qoder**:
  ...
  <Accordion title="Roo Code">
    ...
    Add the Yarah MCP server to your **Roo Code**:
  ...
  <Accordion title="OpenCode">
    ...
    Add the Yarah MCP server to your **OpenCode**:
  ...
  <Accordion title="MCP JSON">
    ...
    Add the Yarah MCP server to your coding agent with MCP JSON:
...
The fastest way to get started is with Remote MCP. A single command attempts installation and tries to handle authentication and project binding. The command may still prompt you for manual steps if the automatic flow cannot complete.
...
Some MCP clients automatically prompt you to log in during setup. Others may require manual authentication or project binding steps if the automatic flow cannot complete. Either way, authentication opens a browser window where you can log in to your Yarah account and grant organization/project access to the MCP client.
...
<AccordionGroup>
  <Accordion title="Cursor">
    ...
    In menu **Preferences → Cursor Settings → Tools & MCP**, you should see Yarah MCP server is added successfully.
    If the Yarah MCP server shows the status "Needs authentication", click the `connect` button to authenticate.
  </Accordion>
  ...
  <Accordion title="Claude Code">
    ...
    claude /mcp
    ```
    Select the "yarah" server, then "Authenticate" to begin the authentication flow.
  ...
  <Accordion title="Cline">
    You can install the Yarah MCP server to your **Cline** by running the following command in your terminal:
    ```bash
    npx -y @smithery/cli mcp add devel/yarah --client cline
    ```
    Alternatively, you can follow the official [Cline documentation](https://docs.cline.bot/mcp/connecting-to-a-remote-server) to add the Yarah MCP server with parameters:
    - Server Name: `yarah`
  ...
  <Accordion title="Roo Code">
    You can install the Yarah MCP server to your **Roo Code** by running the following command in your terminal:
    ```bash
