---
title: 认证 API 参考
description: Yarah 认证 REST API 完整参考，涵盖用户注册、密码登录、邮箱 OTP 无密码登录、邮箱验证、忘记与重置密码、OAuth 第三方登录集成、匿名会话、会话获取与撤销、当前登录用户信息查询以及管理员账号管理等端点的请求参数、字段说明、错误码与响应示例。
---
...
| `redirectTo` | string | 否 | 用于基于链接的电子邮件验证。电子邮件链接始终先打开 Yarah 后端端点；令牌验证后，Yarah 将浏览器重定向到此 URL 并显示验证结果。当 `verifyEmailMethod` 为 `link` 时需要。此 URL 必须包含在 `allowedRedirectUrls` 中。推荐：使用您的应用的登录页面。 |
...
- Yarah 不会在发送验证码时创建用户。如果邮箱尚未注册，只会在验证成功后创建已验证的免密码用户。
...
- 错误：`?yarah_status=error&yarah_type=verify_email&yarah_error=...`
- `yarah_status`：浏览器链接流的结果。对于验证，值为 `success` 或 `error`。
- `yarah_type`：流标识符。对于验证链接，这始终是 `verify_email`。
- `yarah_error`：仅当 `yarah_status=error` 时出现。可显示或记录的人类可读错误消息。
- 推荐处理：使用您的登录页面作为 `redirectTo`。当 `yarah_status=success` 时，显示确认消息并要求用户使用其电子邮件和密码登录。
- 如果 `redirectTo` 未被允许列表，Yarah 会返回 `400` 错误，其消息包括被拒绝的 URL，`nextActions` 告诉您将其添加到 `allowedRedirectUrls`。
...
|-------|------|----------|-------------|
| `email` | string | 是 | 用户电子邮件地址 |
| `redirectTo` | string | 否 | 用于基于链接的密码重置。电子邮件链接始终先打开 Yarah 后端端点；Yarah 然后将浏览器重定向到此 URL，并在查询字符串中包含重置 `token`，以便您的应用可以呈现其自己的重置密码页面。当 `resetPasswordMethod` 为 `link` 时需要。此 URL 必须包含在 `allowedRedirectUrls` 中。推荐：使用您的应用的专用重置密码页面。 |
...
- 准备重置：`?token=...&yarah_status=ready&yarah_type=reset_password`
- 错误：`?yarah_status=error&yarah_type=reset_password&yarah_error=...`
- `token`：仅当 `yarah_status=ready` 时出现。将此值作为 `otp` 传递给 `POST /api/auth/email/reset-password`。
- `yarah_status`：浏览器链接流的结果。对于重置链接，值为 `ready` 或 `error`。
- `yarah_type`：流标识符。对于重置链接，这始终是 `reset_password`。
- `yarah_error`：仅当 `yarah_status=error` 时出现。可显示或记录的人类可读错误消息。
- 您的应用应仅在 `yarah_status=ready` 和 `token` 存在时呈现重置密码表单。
...
<Note>
  仅当额外的查询参数与服务器拥有的 OAuth 字段不冲突时，才会将其作为提供程序特定的提示转发。不要传递 `client_id`、`redirect_uri`、`code_challenge`、`state`、`response_type` 或 `scope`；Yarah/提供程序生成的值获胜，冲突的客户端值被忽略。
</Note>
...
<Note>
`yarah_code` 是一个临时授权码，必须使用 `/api/auth/oauth/exchange` 端点交换为令牌。
</Note>
...
| 字段 | 类型 | 必需 | 描述 |
|-------|------|----------|-------------|
| `code` | string | 是 | 回调中接收的 `yarah_code` |
...
- 如果 `allowedRedirectUrls` 为空，Yarah 允许所有重定向以方便开发者。这对生产不安全，应在本地开发之外避免。
...
| 字段 | 类型 | 必需 | 描述 |
|-------|------|----------|-------------|
| `code` | string | 是 | Yarah Cloud 的授权码或 JWT |
...
刷新仪表板管理员访问令牌。此端点使用仅限仪表板的 `yarah_admin_refresh_token` httpOnly Cookie，不共享应用/用户刷新 Cookie。
