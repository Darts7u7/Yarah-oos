---
title: 数据库 API 参考
description: 通过 Yarah REST API 实现 PostgREST 风格的 Postgres 操作：查询、插入、更新、删除、筛选、排序、分页与 RPC 调用。
---
...
获取成功执行的自定义迁移历史记录。Yarah 会将这些记录存储在 `system.custom_migrations` 中，但该接口只返回迁移的元数据和已执行的语句。**需要管理员身份验证。**
...
| 字段 | 类型 | 是否必需 | 描述 |
|-------|------|----------|-------------|
| `version` | string | 是 | 数字形式的迁移版本号（最多 64 位）。可以是 Drizzle 风格的顺序前缀（例如 `0001`），也可以是 `YYYYMMDDHHmmss` 格式的时间戳。版本号按数字大小比较。Yarah CLI 会为本地迁移文件名生成 14 位的 UTC 时间戳。 |
...
<Warning>
请勿在自定义迁移中包含 `BEGIN`、`COMMIT` 或 `ROLLBACK`。Yarah 会在自己的事务中执行该迁移。
</Warning>
