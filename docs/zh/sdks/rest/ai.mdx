---
title: AI REST 参考
description: 通过 Yarah 模型网关的 HTTP 端点直接调用 OpenRouter，集成任意 LLM 到你的应用，涵盖聊天补全、流式响应、嵌入向量与辅助工具端点。
---

## 概述

新的 AI 集成应直接调用 OpenRouter，使用由 Yarah 提供的 OpenRouter 密钥。Yarah AI 代理路由（用于聊天、图像生成和嵌入）是已弃用的兼容性端点。
...
在 `Authorization` 标头中使用管理员 JWT 持有者令牌或 Yarah API 密钥持有者令牌。
...
此路由验证 Yarah 格式的请求，将其转发到 OpenRouter，并返回 Yarah 格式的响应。它不公开完整的 OpenRouter API 表面。
...
此路由将 Yarah 格式的图像请求转发到 OpenRouter 聊天完成（带图像输出）。仅为了兼容性而保留。
...
此路由将 Yarah 格式的嵌入请求转发到 OpenRouter。对于新代码，更喜欢直接 OpenRouter 嵌入端点。
