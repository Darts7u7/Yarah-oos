...
description: 使用 Yarah Swift SDK 为 iOS、macOS、tvOS 和 watchOS 注册、登录、验证电子邮件、管理会话和配置 OAuth URL 方案。
---
...
对于使用电子邮件注册的用户，Yarah 后端提供三种选择：

1. **不需要电子邮件验证** - 用户可在注册后立即登录。`SignUpResponse` 将具有 `hasSession = true`。
2. **基于链接的验证** - 用户必须打开其电子邮件并单击验证链接才能登录。
3. **基于代码的验证** - Yarah 后端向用户的电子邮件发送 6 位数验证码。客户端应用需要显示验证屏幕，用户可以输入代码，然后调用 `verifyEmail(email:code:)` 完成验证。只有这样用户才能使用电子邮件和密码登录。
...
- `redirectTo` (`String`) - Yarah 身份验证后将重定向到的回调 URL
