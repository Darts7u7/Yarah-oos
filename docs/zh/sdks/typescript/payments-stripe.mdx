---
title: Stripe 支付 SDK
description: 使用 Yarah TypeScript SDK 创建 Stripe Checkout 与账单门户会话，同步产品目录、处理 Webhook 并将订阅状态写入 Postgres。
---
...
SDK 方法需要当前 Yarah 用户令牌。匿名 Yarah 令牌可用于来宾一次性结账，但 Yarah API 密钥不能替代，因为后端需要用户上下文来处理本地会话行。

## createCheckoutSession()

通过 Yarah 后端创建 Stripe Checkout 会话。
...
如果一次性结账包含 `subject` 且还没有现有的 Stripe 客户映射，Yarah 让 Stripe 在 Checkout 期间创建客户，并从完成 webhook 回填主体映射。
...
SDK 方法使用当前 Yarah 令牌调用运行时路由。后端使用调用者上下文插入本地会话行：
...
<Warning>
  永远不要将 Stripe 密钥放在前端代码或公共部署环境变量中。通过 Yarah 仪表板或 CLI 配置 Stripe 密钥。
</Warning>
