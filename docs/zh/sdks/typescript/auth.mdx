---
title: 身份验证 SDK 参考
description: 使用 Yarah TypeScript SDK 实现用户注册、登录、邮箱验证、OAuth 第三方登录、会话管理与用户资料读写，覆盖 Node、浏览器与边缘运行时。
---
...
- `redirectTo` (string, optional) - 用于基于链接的电子邮件验证。电子邮件链接始终首先打开 Yarah 后端端点；在验证令牌后，Yarah 将浏览器重定向到此 URL 以获取验证结果。当 `verifyEmailMethod` 设置为 `link` 时需要。此 URL 必须包含在 `allowedRedirectUrls` 中。推荐：使用您的应用登录页面。
...
<Note>
  当 `requireEmailVerification` 为真时，`accessToken` 将为空直到用户验证其电子邮件。Yarah 发送一个验证电子邮件，其中包含一个链接或 6 位数字代码，具体取决于您的仪表板配置（`verifyEmailMethod`）。对于代码验证，实现一个提示用户输入代码的页面（见 [verifyEmail()](#verifyemail)）。对于链接验证，提供一个在 Yarah 验证令牌后应接收浏览器的 `redirectTo` URL。推荐：使用您的登录页面作为 `redirectTo`，然后显示成功消息并询问用户使用其电子邮件和密码登录。
</Note>
...
<Note>
  OAuth 重定向后，SDK 自动检测回调 `yarah_code`，将其交换为会话，并自动保存会话。
</Note>
...
<Note>
  `additionalParams` 仅用于提供商特定的可选提示。不要传递服务器拥有的 OAuth 字段如 `client_id`、`redirect_uri`、`code_challenge`、`state`、`response_type` 或 `scope`；Yarah 在服务器上设置这些值并忽略冲突的客户端提供的密钥。
</Note>

<Note>
  自定义提供商必须首先在 Yarah 仪表板的 `Auth Methods` 中使用客户端凭证和 OIDC 发现 URL 配置。
</Note>
...
当之前的 OTP 已过期或未收到时，重新发送电子邮件验证。使用身份验证设置中配置的方法（`verifyEmailMethod`）。当方法为 `code` 时，发送 6 位数字代码。当方法为 `link` 时，发送通过 Yarah 后端端点的浏览器验证链接。
...
- `yarah_error`：仅当 `yarah_status=error` 时存在。人类可读的错误消息用于显示或日志。

推荐处理：使用您的登录页面作为 `redirectTo`。当 `yarah_status=success` 时，显示确认消息并询问用户使用其电子邮件和密码登录。
...
使用身份验证设置中配置的方法（`resetPasswordMethod`）发送密码重置电子邮件。当方法为 `code` 时，发送 6 位数字代码用于两步流程。当方法为 `link` 时，发送通过 Yarah 后端端点的浏览器重置链接。
...
- `email` (string, required) - 用户的电子邮件地址
- `redirectTo` (string, optional) - 用于基于链接的密码重置。电子邮件链接始终首先打开 Yarah 后端端点；Yarah 然后将浏览器重定向到此 URL，在查询字符串中包含重置 `token`，以便您的应用可以呈现其自己的重置密码页面。当 `resetPasswordMethod` 设置为 `link` 时需要。此 URL 必须包含在 `allowedRedirectUrls` 中。推荐：使用您的应用的专用重置密码页面。
...
- `token`：仅当 `yarah_status=ready` 时存在。将此值传递给 `resetPassword({ otp })`。
- `yarah_status`：浏览器链接流的结果。对于重置链接，值为 `ready` 或 `error`。
- `yarah_type`：流标识符。对于重置链接，这始终是 `reset_password`。
- `yarah_error`：仅当 `yarah_status=error` 时存在。人类可读的错误消息用于显示或日志。

仅当 `yarah_status=ready` 且 `token` 存在时才呈现重置密码表单。
