...
description: 使用 Yarah TypeScript SDK 通过 Socket.IO 订阅频道、发布事件、追踪在线状态，并处理重新连接和令牌刷新。
---
...
TypeScript SDK 向您的 Yarah 后端打开一个 Socket.IO 连接。您订阅命名频道、监听事件名称，并可选地将事件发布回您已加入的频道。

事件可以来自两个地方：

- 调用 `realtime.publish(channel, event, payload)` 的数据库触发器。
- 调用 `yarah.realtime.publish(channel, event, payload)` 的客户端。
