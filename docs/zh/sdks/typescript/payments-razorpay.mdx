...
description: 使用 Yarah TypeScript SDK 从应用创建 Razorpay 订单与订阅、验证 Webhook 签名，并将支付状态同步到 Postgres 数据库。
---
...
SDK 使用当前 Yarah 用户令牌调用 Razorpay 运行时路由。不要使用提供商密钥调用这些路由。
...
首先创建一个应用拥有的待处理订单。然后通过 Yarah 创建 Razorpay 订单：

Razorpay 订单可以是仅金额的，但为一次性可售产品创建 Razorpay 商品是一个好做法，因为同步的商品使目录在 Yarah 和 Razorpay 中可见。将订单视为付款尝试。
...
首先创建或同步一个 Razorpay 计划。然后通过 Yarah 创建订阅：
