...
description: 通过 Socket.IO 连接，订阅频道，发布事件，跟踪在线状态，并使用 Yarah Kotlin 实时客户端处理重新连接。
---
...
Kotlin SDK 通过 Socket.IO 连接到 Yarah 实时系统。订阅频道名称（如 `order:123` 或 `chat:room-1`），然后监听发布到这些频道的事件名称。
