---
title: 代码配置
description: 通过 yarah.toml 声明文件与 CLI 的 plan、apply、export 工作流，代码化管理 Yarah 身份验证、SMTP、存储、保留策略与部署配置。
---

## 概览

`yarah.toml` 是项目配置子集的声明性版本控制快照：身份验证策略、允许的重定向 URL、密码规则、SMTP、存储上传大小、实时和计划保留以及部署子域。CLI 提供针对此文件的三个命令：

- **`config plan`**：将 `yarah.toml` 与链接项目进行比较。显示会发生什么变化。
- **`config apply`**：将差异推送到实时项目。按更改能力门控、环境解析的秘密、干运行模式。
- **`config export`**：拉取当前项目状态并写新的 `yarah.toml`。对从现有项目引导很有用。

您在存储库中保留**一个** `yarah.toml`。要将其应用于不同环境（暂存、生产、队友的本地后端或自托管实例），请将 CLI 指向不同的项目（重新链接或使用 `--project-id`）。秘密通过 `env(...)` 引用从环境变量读取，所以文件本身保持免费的凭证并可以安全提交。

这在 Yarah Cloud 项目和自托管 OSS 部署上的工作方式相同。

<Note>
  所有示例使用 `npx @yarah/cli`。不要全局安装 CLI。
</Note>
...
| 命令 | 目的 |
|---------|---------|
| `config plan` | 显示 `yarah.toml` 和实时项目状态之间的差异 |
| `config apply` | 将 `yarah.toml` 应用于实时项目 |
| `config apply --dry-run` | 打印计划而不应用 |
| `config apply --auto-approve` | 跳过交互式确认提示（在 `--json` 模式下需要） |
| `config export` | 拉取实时配置并写 `yarah.toml` |
| `config export --force` | 不确认覆盖现有 `yarah.toml` |

`config plan` 和 `config apply` 读取 `yarah.toml`；如果它存在于其他位置，通过 `--file <path>`。`config export` 写入文件；通过 `--out <path>` 将其写入自定义位置。
...
<Steps>
  <Step title="从现有项目引导">
    ...
    这写 `yarah.toml` 反映当前后端状态。
  </Step>

  <Step title="提交它">
    检查 `yarah.toml` 进入版本控制。秘密通过 `env(...)` 引用，所以文件很安全。
  </Step>
...
`plan` 读取 `yarah.toml`，通过 `/api/metadata` 和存储、实时和计划的可选配置端点获取实时状态，然后打印呈现的差异。它也标记任何部分实时后端尚未公开（较旧的自托管版本等）。应用将跳过而不是失败整个运行。
...
这是让一个 `yarah.toml` 清洁地应用到多个环境的原因：开发和生产后端仅在**哪个** `SENDGRID_API_KEY` 在您运行 `apply` 时范围内有所不同。
...
- **灾难恢复。** 提交的 `yarah.toml` 是已知好的配置快照。在恢复项目后重新应用它以在几秒钟内使身份验证、SMTP、存储、保留和部署设置回到预期形状。
- **自托管和本地 OSS 开发。** 运行 `npx @yarah/cli link --api-base-url http://localhost:7130 --api-key <local-key>` 针对 docker-compose 堆栈，然后将 CLI 的配置命令指向您的本地 OSS 实例，方式与您指向云的方式相同。
