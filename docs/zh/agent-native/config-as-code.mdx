...
<Note>
  所有示例使用 `npx @yarahdev/cli`。不要全局安装 CLI。
</Note>
...
- **自托管和本地 OSS 开发。** 运行 `npx @yarahdev/cli link --api-base-url http://localhost:7130 --api-key <local-key>` 针对 docker-compose 堆栈，然后将 CLI 的配置命令指向您的本地 OSS 实例，方式与您指向云的方式相同。
