---
title: "将 Yarah 部署到 Containarium"
description: "在 Containarium LXC 主机上自托管 Yarah，利用按租户容器、ZFS 快照与 MCP 驱动的配置流程，为代理原生工作流提供轻量级、可复制的部署方案。"
---

# 将 Yarah 部署到 Containarium

本指南将逐步介绍如何在 [Containarium](https://github.com/footprintai/containarium) 主机上部署 Yarah。Containarium 是一个开源、可自托管的平台，为每个租户提供一个持久的 Linux 容器（LXC），具备一流的 SSH、MCP 和基于主机名的 TLS 原语——非常适合代理驱动的 Yarah 部署。

<Note>
  本指南由社区维护，可能落后于最新的 Yarah 版本。规范的、始终保持最新的配置位于 [Yarah 仓库](https://github.com/Darts7u7/Yarah-oos)中的 `deploy/docker-compose/` 目录。
</Note>

## 何时选择 Containarium

如果你需要以下特性，Containarium 非常适合 Yarah 部署：

- **自托管的多租户基础设施**：在一台主机上运行多个相互隔离的 Yarah 项目，每个项目都在自己的 LXC 中，每个项目有一个 TLS 主机名——无需共享 `docker compose -p` 记账。
...
每个 Yarah box 的最低配置：**2 个 vCPU、4 GB 内存、30 GB 磁盘**。
...
`--stack docker` 标志会在容器内安装 Docker CE 和 compose 插件。配置你的 SSH，使 `ssh yarah` 可用：
...
会 checkout 这个栈要读的文件，并把密钥生成到 `~/yarah/.env`。不启动任何东西。

### 3. 配置环境

在 box 内编辑 `~/yarah/.env`。至少需要设置：
...
完整列表（OpenRouter、OAuth 提供商、Stripe、Vercel）请参见 [`.env.example`](https://github.com/yarah/yarah/blob/main/.env.example)。
...
你应该会看到 `4/4 services up`：`postgres`、`postgrest`、`yarah`、`deno`。（compose 文件为 `postgres`、`postgrest` 和 `deno` 提供了健康检查；`yarah` 会在其他服务健康且自身已启动后报告 `Up`。）

### 5. 通过公共主机名对外暴露

Yarah 默认在 7130 端口提供仪表盘和 API 服务。
...
这会将 Containarium sentinel 上的 Caddy 配置为终止 `<your-subdomain>` 的 TLS 并转发到 Yarah 容器。证书会在首次请求时通过 ACME 自动配置——无需 certbot，无需 nginx 配置。
...
### 6. 将你的代理连接到 Yarah MCP

在浏览器中打开 `https://<your-subdomain>`，按照产品内的流程将你的支持 MCP 的代理（Cursor、Claude Code、Windsurf、OpenCode 等）连接到 Yarah MCP 服务器。
...
或按服务查看：`docker compose logs -f yarah` / `postgres` / `deno`。
...
Yarah 的四个服务在空闲时大约需要 3 GB 常驻内存。如果你将 box 配置为 2 GB，请调整大小：
...
- **GPU 直通**：Containarium 支持它，但 Yarah 内置的边缘函数不使用 GPU。除非你的自定义 Deno 函数需要 GPU，否则请保持关闭。
...
- 对于生产环境，请选择使用 Containarium 的 KMS 信封加密（Vault Transit 或 GCP KMS）来保护存储在 Containarium 密钥库中的任何 Yarah 密钥。
