---
title: OAuth 服务器
description: 使用 Yarah 作为 OAuth 2.0 身份提供商，在第三方应用中验证用户，涵盖客户端注册、授权流程、令牌颁发以及作用域配置的完整集成指南。
---

## 概述

Yarah 可以作为 OAuth 2.0 身份提供商，允许第三方应用程序通过"使用 Yarah 登录"来验证用户身份。这使得在您的平台上进行开发的开发者能够利用 Yarah 的身份验证系统，而无需管理自己的用户凭据。

## 使用场景

<CardGroup cols={2}>
  <Card title="开发者平台" icon="code">
    允许第三方开发者使用"使用 Yarah 登录"构建集成，同时您仍保持对用户数据访问的控制。
  </Card>
  ...
  <Card title="合作伙伴应用" icon="handshake">
    允许合作伙伴应用程序针对您的 Yarah 项目验证用户身份，而无需共享凭据。
  </Card>
...
Yarah 实现了带 PKCE（Proof Key for Code Exchange，代码交换验证密钥）的**授权码流程**（Authorization Code flow），这是适用于 Web 和原生应用程序的最安全的 OAuth 流程。
...
<Steps>
  <Step title="注册您的应用程序">
    联系 Yarah 将您的应用程序注册为 OAuth 客户端。您将获得：
...
<Card title="OAuth 示例仓库" icon="github" href="https://github.com/Darts7u7/Yarah-oauth-example">
  完整可运行的示例，展示如何将"使用 Yarah 登录"集成到您的应用程序中。
</Card>
