...
`@yarah/cli posthog setup` 将您的 Yarah 项目链接到 PostHog，然后打印官方 [PostHog 向导](https://posthog.com/docs/libraries/wizard) 命令 (`npx -y @posthog/wizard@latest`) 供您（或您的代理）接下来运行。向导检测您的框架，安装正确的 PostHog SDK，并放入初始化代码，以便页面浏览、自动捕获事件和会话录制开始流动。
...
<CardGroup cols={2}>
  ...
  <Card title="Yarah CLI" icon="terminal" href="/quickstart">
    `npx @yarah/cli posthog setup` 将您的 Yarah 项目链接到 PostHog，然后打印向导命令。
  </Card>
</CardGroup>
