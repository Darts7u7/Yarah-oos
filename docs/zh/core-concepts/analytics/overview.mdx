...
description: "由 PostHog 驱动的应用产品分析：追踪用户事件、留存漏斗、功能标志与会话回放，直接嵌入 Yarah 仪表板，无需额外埋点服务。"
---

使用 Yarah 分析来了解人们如何实际使用您的应用：页面流量、保留和会话回放，所有这些都通过将 PostHog 项目连接到您的 Yarah 后端来进行。连接后，仪表盘在您的 PostHog 数据上呈现流量、用户保留和会话回放页面，无需离开 Yarah。
...
<Frame caption="分析仪表盘：随着时间推移的 KPI 加上按页面、国家和设备的细分。">
  <img src="/images/dashboard-analytics.png" alt="Yarah 分析仪表板显示访客 KPI、访客趋势图表以及顶级页面、国家和设备细分" />
</Frame>

<Note>
  PostHog 仍然是事件、仪表盘、见解和录制的真实来源。Yarah 为日常检查呈现一个集中的子集，然后对于超出其范围的任何内容都深入链接到 PostHog。
</Note>
...
从仪表盘中的分析页面连接 PostHog。Yarah 为您配置或链接 PostHog 项目，存储凭证到服务器端，并在连接成功后解锁流量、保留和会话回放页面。
...
`@yarahdev/cli posthog setup` 将您的 Yarah 项目链接到 PostHog，然后打印官方 [PostHog 向导](https://posthog.com/docs/libraries/wizard) 命令 (`npx -y @posthog/wizard@latest`) 供您（或您的代理）接下来运行。向导检测您的框架，安装正确的 PostHog SDK，并放入初始化代码，以便页面浏览、自动捕获事件和会话录制开始流动。
...
分析配置对话（侧栏中的齿轮图标）让管理员审查链接的 PostHog 项目，直接跳转到 PostHog，并在需要时断开连接。断开连接只会断开 Yarah ↔ PostHog 链接；您的 PostHog 项目、事件和录制保持不变。
...
<CardGroup cols={2}>
  ...
  <Card title="Yarah CLI" icon="terminal" href="/quickstart">
    `npx @yarahdev/cli posthog setup` 将您的 Yarah 项目链接到 PostHog，然后打印向导命令。
  </Card>
</CardGroup>
