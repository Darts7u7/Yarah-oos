...
description: "在 Yarah 项目旁运行长期存活的容器，用于队列 worker、AI 推理、websocket 服务或爬虫。"
---

用 Yarah Custom Compute 在项目旁运行长期存活的容器：队列 worker、后台处理器、AI 推理循环、websocket 服务、爬虫，任何需要持续运行的进程。
...
把任何 Docker 镜像交给 Yarah 就能跑起来。可以指向 registry 上已构建好的镜像，也可以上传构建上下文，让 Yarah 用你的 `Dockerfile` 来构建。不需要学习额外的构建流水线。

### 访问你的项目

容器需要的凭据由你以服务的环境变量形式设置——项目 URL、API key、S3 凭据，取决于这个工作负载用到什么。Yarah 不会替你注入任何东西，因此容器能访问什么完全由你决定。
...
在 Yarah Cloud 上 compute 完全托管，你无需配置。自托管时，由你决定容器跑在哪里。目前有两种 provider，在配置好其中一种之前，compute 接口都会返回 `503 COMPUTE_NOT_CONFIGURED`。

<Tabs>
  <Tab title="Docker（你自己的宿主机）">
    容器跑在运行 Yarah 的同一个 Docker daemon 上，与 Yarah 容器互为兄弟容器。不需要注册任何服务，也没有按容器计费。

    启用方式是一个明确的动作：把 Docker socket 挂载进 Yarah 容器。compose 文件里已经为此准备好了一行，取消注释即可：
    ...
    <Warning>
      **Docker socket 在宿主机上等同于 root。** 能访问它的人都可以启动一个读取整个文件系统的容器，所以挂载它应当是一个有意识的决定。Yarah 自己构造每个容器的 spec，从不转发调用方提供的选项，因此即使 Yarah API key 泄漏，攻击者也无法要求特权容器或宿主机 bind mount——但 socket 本身的权限，依然等同于拥有它的账号。
    </Warning>
  ...
  <Tab title="Fly.io">
    ...
    - `FLY_API_TOKEN`：Fly.io API token，用 `fly tokens create org -o <your-org>` 生成。把 CLI 打印的整行都粘进来，`FlyV1` 前缀会自动处理。Yarah 用它来创建和管理 compute 容器。
...
| `port` | 发布在 daemon 分配的宿主机端口上。设置 `COMPUTE_PUBLIC_HOST` 后 Yarah 才会公布 URL；留空则不返回 URL，而不是给出一个可能无法解析的地址。 |
...
用 `COMPUTE_DEFAULT_INGRESS` 设置整个部署的默认值。`host` 模式下 Yarah 只公布主机名，不负责 TLS 终止和流量路由——请像你已经为 dashboard 做的那样，在前面自己跑一个网关（Caddy、Traefik、nginx）。

### 从源码构建

部署已构建好的镜像不需要额外步骤：用镜像地址创建服务，它就会被拉取并启动。

要让 Yarah 构建你的 `Dockerfile`，先占位服务，再把构建上下文作为 tar 包上传：
...
| 源码构建 | 向 Yarah 上传上下文 | 由 CLI 用 `flyctl` 构建 |
