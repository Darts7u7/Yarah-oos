...
description: "将 Razorpay 订单、订阅与 Webhook 履约集成到 Yarah：通过 SDK 与仪表板配置密钥、验证签名并同步支付状态到 Postgres 数据库。"
---
...
Razorpay 没有相当于 Stripe Billing Portal 的功能。Yarah 提供后端路由，在依据 `payments.razorpay_subscriptions` 的 RLS `UPDATE` 策略检查调用方之后，用于取消、暂停和恢复订阅。

对于一次性产品，创建 Razorpay Order 时只需提供金额、货币和 receipt 即可。不过，仍建议为可销售的产品创建 Razorpay Item，这样在同步之后，产品目录就能在 Yarah 和 Razorpay 中可见。请将 Order 视为一次支付尝试，而非您的产品目录。

## 设置

在 Dashboard -> Payments -> Settings 中或通过管理 API 配置 `test` 和 `live` 的 Razorpay Key ID 与 Key Secret。如果尚不存在 Webhook 签名密钥，Yarah 会自动生成一个。
...
首先创建一个由应用程序拥有的待处理订单（pending order）。然后使用当前 Yarah 用户令牌创建一个 Razorpay Order。
