...
description: "Integra Razorpay Orders, Subscriptions, webhooks manuales y cumplimiento de webhooks con Yarah para cobrar pagos, suscripciones y facturación en tu app."
---
...
| Concepto de Razorpay | Significado en Yarah |
|------------------|---------------------|
...
Razorpay no tiene un equivalente al Stripe Billing Portal. Yarah expone rutas de backend para cancelar, pausar y reanudar suscripciones tras verificar al llamador contra las políticas RLS `UPDATE` de `payments.razorpay_subscriptions`.

Para productos de pago único, los Razorpay Orders pueden crearse solo con un importe, una moneda y un recibo (receipt). Aun así, es preferible crear Razorpay Items para los productos vendibles, de modo que el catálogo sea visible en Yarah y Razorpay tras la sincronización. Trata los Orders como intentos de pago, no como tu catálogo de productos.

## Configuración

Configura el Key ID y el Key Secret de Razorpay para `test` y `live` en Dashboard -> Payments -> Settings, o mediante la API de administración. Yarah genera un secreto de firma de webhook si aún no existe uno.
...
4. Selecciona los eventos que Yarah gestiona.
...
Primero crea un pedido pendiente (pending order) propio de la app. Luego crea un Razorpay Order con el token de usuario actual de Yarah.
