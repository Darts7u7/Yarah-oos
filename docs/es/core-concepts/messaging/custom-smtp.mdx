...
description: Configure su propio servidor SMTP para enviar correos de autenticación y transaccionales de Yarah, con verificación y enlaces mágicos.
---
...
<Note>
  **Las instancias autohospedadas necesitan esto para poder enviar cualquier correo electrónico.** El remitente gestionado sin configuración se enruta a través de Yarah Cloud y solo está disponible para proyectos vinculados a la nube. Una instancia autohospedada sin SMTP configurado no puede entregar correos de autenticación, por lo que la verificación de correo electrónico, los enlaces mágicos y los restablecimientos de contraseña no se entregarán hasta que complete la configuración a continuación.
</Note>

## Conceptos

El proveedor se resuelve en cada envío, por lo que los cambios surten efecto en la siguiente solicitud. Yarah ejecuta `transporter.verify()` antes de guardar, por lo que una configuración persistente siempre funciona. Las contraseñas se cifran en reposo con AES-256-GCM y nunca las devuelve la API.
...
<Steps>
  ...
  <Step title="Guardar">
    Yarah ejecuta un apretón de manos SMTP antes de persistir. Las malas credenciales fallan rápidamente.
  </Step>
