...
description: "Ejecute funciones perimetrales TypeScript sin servidor sobre Deno con invocación HTTP, cron, activadores de base de datos, secretos y variables de entorno."
---

Utilice Yarah Edge Functions para ejecutar TypeScript en [Deno](https://deno.com), implementado cerca de sus usuarios para baja latencia. Las funciones se pueden invocar bajo demanda desde cualquier cliente, encadenarse desde activadores de base de datos o programarse para ejecutarse en una expresión cron. El tiempo de ejecución incluye búsqueda estándar, respuestas de transmisión e importaciones ESM listos para usar.
...
Cada función es accesible en `https://<project>.yarah.dev/functions/<name>`. Búsqueda estándar de entrada, salida estándar `Response`. La transmisión, JSON, redirecciones y websockets funcionan.

### Calendarios

Adjunte una expresión cron a una función e Yarah la invoca a tiempo, con reintentos en caso de fallo. Vea [Schedules](/core-concepts/functions/schedules) para la sintaxis cron y el modelo de ejecución.
...
La diferencia está en cómo se almacena y protege cada valor. Los **secretos** se cifran en reposo: `secrets list` muestra solo metadatos con los valores ocultos, y `secrets get` devuelve el valor descifrado cuando lo necesita. Un secreto también puede llevar una fecha de expiración (`--expires <ISO-8601>`) y marcarse como protegido contra eliminación (`--reserved`). Las **variables de entorno** son valores de configuración simples, sin cifrado ni expiración. Ponga credenciales como claves de API y contraseñas en secretos, y la configuración no sensible en variables de entorno. Ambos quedan disponibles para el código de su función de la misma manera.

```bash
# un secreto protegido con expiración
npx @yarahdev/cli secrets add STRIPE_SECRET_KEY sk_live_xxx --reserved --expires 2027-01-01T00:00:00Z
# lea de vuelta el valor descifrado
npx @yarahdev/cli secrets get STRIPE_SECRET_KEY
```

### Registros

Los registros estructurados se capturan por invocación, consultables por estado, duración y nombre de función. La herramienta Yarah MCP `get-function-logs` permite que su agente diagnostique fallos sin salir del editor.
