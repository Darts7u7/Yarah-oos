...
description: "Implementa aplicaciones frontend desde tu proyecto de Yarah con la potencia de Vercel: dominios personalizados, CDN global y despliegues por rama de git."
---

Usa Yarah Sites para publicar la aplicación orientada al navegador que pertenece a tu proyecto. El CLI de Yarah sube el código fuente de tu frontend a través de Yarah, que crea una implementación de producción en Vercel. El panel realiza el seguimiento de la URL, el estado, el historial de implementaciones, las variables de entorno y los dominios.

<Frame caption="Panel de Sites: estado, dominios, variables de entorno e historial de implementaciones.">
  <img src="/images/dashboard-sites.png" alt="Panel de Yarah Sites" />
</Frame>
...
Implementa desde el directorio fuente de tu aplicación. El CLI sube el árbol de archivos fuente, omite los archivos exclusivamente locales como `node_modules`, `.git`, la salida de compilación y los archivos `.env`, y luego inicia la compilación en Vercel a través de Yarah.
...
Implementa proyectos en React, Vue, Svelte, Next.js, sitios estáticos y otros proyectos frontend. Yarah envía los archivos fuente a Vercel, donde la detección de framework y los archivos del proyecto, como `package.json` y `vercel.json`, determinan cómo se compila la aplicación.
...
Cada implementación lista obtiene una URL predeterminada en `https://<appkey>.yarah.site`. También puedes configurar un slug gestionado por Yarah en `https://<slug>.yarah.site`. Para un dominio personalizado, agrega el dominio en el panel y configura el registro DNS que este devuelve, normalmente un CNAME para subdominios.

## Impleméntalo

<CardGroup cols={2}>
  <Card title="Guía rápida del CLI" icon="terminal" href="/quickstart">
    Conecta tu proyecto y ejecuta comandos del CLI de Yarah desde el directorio de tu aplicación.
  </Card>
</CardGroup>
...
- Agrega variables de entorno seguras para el navegador desde el panel o con `npx @yarah/cli deployments env set`.
- Ejecuta `npx @yarah/cli deployments deploy ./frontend`.
