...
description: "Administra tus datos con la base de datos Postgres de Yarah: tablas, PostgREST, Row Level Security, pgvector, migraciones y copias de seguridad diarias."
---

Cada proyecto de Yarah viene con una base de datos [Postgres](https://www.postgresql.org/) completa. Cada tabla es automáticamente un punto final REST y SDK escrito. Los tokens de autenticación limitan cada lectura y escritura a través de seguridad a nivel de fila. El mismo Postgres maneja consultas relacionales, búsqueda semántica a través de pgvector y fuentes de cambios en tiempo real.

<Frame caption="El editor de tablas: columnas escrita, edición en línea, importación CSV y estudio SQL.">
  <img src="/images/database-table-editor.png" alt="Editor de tablas del panel de Yarah que muestra una tabla de mensajes con columnas escritas" />
</Frame>
...
Rastrear y aplicar cambios SQL en orden. [Migrations](/core-concepts/database/migrations) se envían como archivos `.sql` simples en su repositorio, aplicados con `npx @yarahdev/cli db migrations up --all` o mediante la herramienta MCP.
