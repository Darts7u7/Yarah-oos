...
description: Gestiona configuración de autenticación, SMTP, almacenamiento y despliegue de Yarah de forma declarativa desde yarah.toml usando la CLI con plan y apply.
---

## Descripción general

`yarah.toml` es una instantánea de control de versiones declarativa de un subconjunto de la configuración de su proyecto: política de autenticación, URL de redirección permitidas, reglas de contraseña, SMTP, tamaño de carga de almacenamiento, retención en tiempo real y programación, y subdominio de implementación. La CLI proporciona tres comandos que funcionan con este archivo:

- **`config plan`**: diferencie `yarah.toml` con el proyecto vinculado. Muestra qué cambiaría.
...
- **`config export`**: extraiga el estado del proyecto actual y escriba un nuevo `yarah.toml`. Útil para arrancar desde un proyecto existente.

Usted mantiene **un** `yarah.toml` en su repositorio. Para aplicarlo a un entorno diferente (staged, producción, backend local de un compañero de equipo, o instancia autohospedada), apunte la CLI a un proyecto diferente (revinculación o use `--project-id`). Los secretos se leen de variables de entorno a través de referencias `env(...)`, por lo que el archivo en sí permanece libre de credenciales y puede ser confirmado de forma segura.

Esto funciona de la misma manera en los proyectos de Yarah Cloud y en implementaciones OSS autohospedadas.

<Note>
  Todos los ejemplos usan `npx @yarah/cli`. No instale la CLI globalmente.
</Note>
...
| Comando | Propósito |
|---------|---------|
| `config plan` | Mostrar diferencia entre `yarah.toml` y estado del proyecto en vivo |
| `config apply` | Aplicar `yarah.toml` al proyecto en vivo |
...
| `config export` | Extraer configuración activa y escribir `yarah.toml` |
| `config export --force` | Sobrescribir un `yarah.toml` existente sin confirmación |

`config plan` y `config apply` leen `yarah.toml`; pase `--file <path>` si se encuentra en otro lugar. `config export` escribe el archivo; pase `--out <path>` para escribirlo en una ubicación personalizada.
...
<Steps>
  <Step title="Arranque desde un proyecto existente">
    ...
    Esto escribe `yarah.toml` reflejando el estado del backend actual.
  </Step>

  <Step title="Confirmarlo">
    Verifique `yarah.toml` en control de versiones. Los secretos se hacen referencia a través de `env(...)`, por lo que el archivo es seguro.
  </Step>
...
- `--file <path>` anula la ubicación predeterminada `./yarah.toml`.
...
- Arrancar un `yarah.toml` desde un proyecto que ha estado configurando a través del panel.
...
Esto es lo que permite que un `yarah.toml` se aplique limpiamente a múltiples entornos: los backends de desarrollo y producción difieren solo en **cuál** `SENDGRID_API_KEY` está en el alcance cuando ejecuta `apply`.
...
- **Recuperación ante desastres.** Un `yarah.toml` confirmado es una instantánea de configuración conocida. Vuelva a aplicarlo después de restaurar un proyecto para que los ajustes de autenticación, SMTP, almacenamiento, retención e implementación vuelvan a la forma esperada en segundos.
- **Desarrollo OSS autohospedado y local.** Ejecute `npx @yarah/cli link --api-base-url http://localhost:7130 --api-key <local-key>` contra una pila docker-compose, luego apunte los comandos de configuración de la CLI a su instancia OSS local de la misma manera que apuntaría a cloud.
