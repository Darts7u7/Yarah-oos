...
<Note>
  Todos los ejemplos usan `npx @yarahdev/cli`. No instale la CLI globalmente.
</Note>
...
- **Desarrollo OSS autohospedado y local.** Ejecute `npx @yarahdev/cli link --api-base-url http://localhost:7130 --api-key <local-key>` contra una pila docker-compose, luego apunte los comandos de configuración de la CLI a su instancia OSS local de la misma manera que apuntaría a cloud.
