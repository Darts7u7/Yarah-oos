...
description: Integra Yarah como plataforma de socio mediante asociaciones de marca compartida o marca blanca con panel embebido, gestión del ciclo de vida y OAuth.
---

## Descripción general

Yarah ofrece dos modelos de asociación que permiten integrar de forma fluida nuestra plataforma de Backend as a Service en sus aplicaciones. Ya sea que busque una solución de marca compartida o una experiencia totalmente de marca blanca, tenemos el modelo de asociación adecuado para usted.

## Modelos de asociación

<CardGroup cols={2}>
  <Card title="Asociación de marca compartida" icon="handshake">
    Perfecto para plataformas que desean ofrecer Yarah junto con sus propios servicios.
    El proceso de integración es sumamente sencillo: los desarrolladores pueden conectarse a la plataforma Yarah con un solo clic, sin flujos de OAuth complicados.
  </Card>
...
<Steps>
  <Step title="Solicitar la asociación">
    Envíe su solicitud de asociación a través de nuestro [correo de asociaciones](mailto:partnerships@yarah.dev).
  ...
  <Step title="Configurar la autenticación">
    ...
    ```bash
    curl -X POST https://api.yarah.dev/partnership/v1/YOUR_PARTNER_ID/endpoint \
...
En el modo de marca compartida, los desarrolladores saben explícitamente que son usuarios de la plataforma Yarah (vinculados mediante la misma dirección de correo electrónico). Después de iniciar sesión en la plataforma Yarah, los desarrolladores tienen derechos de gestión completos sobre los proyectos creados a través de socios y deben pagar de acuerdo con los planes de facturación de Yarah.

Las plataformas de socios pueden:
- Sincronizar cuentas de usuario (nombre, correo electrónico) con Yarah
- Sincronizar proyectos con Yarah
...
En el modo de marca blanca, los desarrolladores no tienen conocimiento de la existencia de Yarah y no pueden ver los proyectos creados por socios en la plataforma Yarah.
...
<Tabs>
  <Tab title="APIs de marca compartida">
    ...
    Sincroniza la información de la cuenta de usuario con Yarah.
    ...
    **Respuesta:**
    ```json
    {
      ...
      "project": {
        "id": "uuid-string",
        "access_host": "https://project.us-east.apps.yarah.dev",
    ...
    Nota: debido a los límites de proyectos en el plan gratuito de Yarah, la sincronización de proyectos desde las plataformas de socios puede fallar. En tales casos, la respuesta será:
    ...
    ```json
    {
      ...
      "candidate_projects": [
        {
          "id": "uuid-string",
          "access_host": "https://project2.us-east.apps.yarah.dev",
          "api_key": "project-api-key",
          "status": "active"
        }
      ]
...
sequenceDiagram
    ...
    participant Yarah as Yarah API
    participant Dashboard as Yarah Dashboard

    User->>Partner: Sign up/Login
    Partner->>Yarah: POST /connect-user<br/>(name, email)
    Yarah-->>Partner: Return user account ID

    User->>Partner: Create new project
    Partner->>Yarah: POST /sync-project<br/>(userId, project_name)

    alt Success
        Yarah-->>Partner: Return project details<br/>(id, access_host, api_key)
        Partner->>Partner: Store project credentials
        Partner->>User: Show project ready
    else Project Limit Reached
        Yarah-->>Partner: Return error + candidate projects
        Partner->>User: Show existing projects to choose
        User->>Partner: Select existing project
        Partner->>Yarah: GET /metadata<br/>(projectId)
        Yarah-->>Partner: Return project details
    end

    User->>Partner: Use application features
    Partner->>Yarah: API calls using project credentials
    Yarah-->>Partner: Return data
    Partner-->>User: Display results

    Note over User,Dashboard: User can also access Yarah Dashboard directly
...
const connectUser = async (name: string, email: string) => {
  const response = await fetch(
    `https://api.yarah.dev/partnership/v1/${PARTNER_ID}/connect-user`,
    {
      method: 'POST',
      headers: {
...
const createProject = async (userId: string, projectName: string) => {
  const response = await fetch(
    `https://api.yarah.dev/partnership/v1/${PARTNER_ID}/${userId}/sync-project`,
    {
      method: 'POST',
      headers: {
        'X-Partnership-Secret': `${SECRET_KEY}`,
...
sequenceDiagram
    ...
    Note over User,Yarah: User never interacts with Yarah directly

    User->>Partner: Sign up/Login to partner platform

    User->>Partner: Create new project
    Partner->>Yarah: POST /sync-embedded-project<br/>(project_name, region, instance_type)
    Yarah-->>Partner: Return project details<br/>(id, access_host, api_key, status)
    Partner->>Partner: Store project credentials
    Partner->>User: Show project ready
    ...
    Partner->>Yarah: POST /pause<br/>(projectId)
    Yarah-->>Partner: Return status: paused
    Partner->>User: Confirm project paused

    User->>Partner: Request to restore project
    Partner->>Yarah: POST /restore<br/>(projectId)
    Yarah-->>Partner: Return status: active
    Partner->>User: Confirm project active

    Partner->>Yarah: GET /metadata<br/>(projectId)
    Yarah-->>Partner: Return current project status
```
...
const createEmbeddedProject = async (projectName: string) => {
  const response = await fetch(
    `https://api.yarah.dev/partnership/v1/${PARTNER_ID}/sync-embedded-project`,
    {
      method: 'POST',
      headers: {
        'X-Partnership-Secret': `${SECRET_KEY}`,
...
const pauseProject = async (projectId: string) => {
  const response = await fetch(
    `https://api.yarah.dev/partnership/v1/${PARTNER_ID}/${projectId}/pause`,
    {
      method: 'POST',
      headers: {
        'X-Partnership-Secret': `${SECRET_KEY}`,
...
const restoreProject = async (projectId: string) => {
  const response = await fetch(
    `https://api.yarah.dev/partnership/v1/${PARTNER_ID}/${projectId}/restore`,
    {
      method: 'POST',
      headers: {
        'X-Partnership-Secret': `${SECRET_KEY}`,
...
const deleteProject = async (projectId: string) => {
  const response = await fetch(
    `https://api.yarah.dev/partnership/v1/${PARTNER_ID}/${projectId}`,
    {
      method: 'DELETE',
      headers: {
        'X-Partnership-Secret': `${SECRET_KEY}`
...
const getPartnershipUsage = async (startDate: string, endDate: string) => {
  ...
  const response = await fetch(
    `https://api.yarah.dev/partnership/v1/${PARTNER_ID}/usage?${params}`,
    {
      method: 'GET',
      headers: {
        'X-Partnership-Secret': `${SECRET_KEY}`
...
Yarah ofrece capacidades de implementación global. Cada proyecto puede implementarse en diferentes regiones:
...
Según los requisitos de negocio de su proyecto, Yarah ofrece los siguientes tipos de recursos:
...
- Programe una [revisión técnica](https://calendly.com/tony-chang-yarah/45min) con nuestro equipo
