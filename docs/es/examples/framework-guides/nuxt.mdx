---
title: Nuxt
description: Aprende a crear un proyecto en Yarah y desarrollar una aplicación Nuxt con IA, autenticación, base de datos Postgres y almacenamiento con el SDK oficial.
---

import Installation from '/snippets/sdk-installation.mdx';

Aprende a crear un proyecto de Yarah y a desarrollar una aplicación Nuxt utilizando herramientas de IA como Cursor.

## 1. Crear un proyecto de Yarah

Crea un nuevo proyecto de Yarah en [yarah.dev](https://yarah.dev).

## 2. Conectar Yarah

Dos partes:

- **Enlace de la CLI** — consulta la [Guía rápida](/quickstart) para ejecutar `npx @yarahdev/cli link --project-id <your-project-id>` y que el agente pueda leer el ID de tu proyecto y tus claves.
...
Install the Yarah SDK and set up the client configuration.

In my Yarah database, create a sports table with id and name columns.
...
export default defineNuxtConfig({
  runtimeConfig: {
    public: {
      yarahBaseUrl: process.env.NUXT_PUBLIC_YARAH_BASE_URL,
      yarahAnonKey: process.env.NUXT_PUBLIC_YARAH_ANON_KEY
    }
  }
})
```
...
export default defineEventHandler(async (event) => {
  ...
  const client = createClient({
    baseUrl: config.public.yarahBaseUrl,
    anonKey: config.public.yarahAnonKey
  })
...
```
Add images to each sport using Yarah Storage.
...
Use Yarah AI with streaming responses.
```
