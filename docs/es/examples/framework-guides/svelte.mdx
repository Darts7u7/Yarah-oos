---
title: Svelte
description: Aprende a crear un proyecto en Yarah y desarrollar una aplicación Svelte con IA, autenticación, base de datos Postgres y almacenamiento con el SDK oficial.
---

import Installation from '/snippets/sdk-installation.mdx';

Aprende a crear un proyecto de Yarah y a desarrollar una aplicación Svelte utilizando herramientas de IA como Cursor.
...
**Cliente de Yarah** en `src/lib/yarah.ts`:

```typescript src/lib/yarah.ts
import { createClient } from '@yarahdev/sdk';

export const yarah = createClient({
  baseUrl: 'https://your-project.us-east.apps.yarah.dev',
...
<script lang="ts">
  ...
  onMount(async () => {
    try {
      const { data, error: fetchError } = await yarah.database
        .from('sports')
        .select();
