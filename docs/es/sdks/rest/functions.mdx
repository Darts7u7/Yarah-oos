...
description: Invoca funciones sin servidor edge de Yarah mediante la API REST desde cualquier cliente HTTP para ejecutar lógica de servidor, webhooks y tareas asíncronas.
---
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/functions/hello-world" \
...
```bash
curl "https://your-app.apps.yarah.dev/api/functions" \
...
```bash
curl "https://your-app.apps.yarah.dev/api/functions/hello-world" \
  -H "Authorization: Bearer admin-jwt-token-or-api-key"
```
...
<Note>
Actualmente, Yarah solo admite funciones JavaScript/TypeScript que se ejecutan en un entorno Deno.
</Note>
...
```bash
curl -X POST "https://your-app.apps.yarah.dev/api/functions" \
  -H "Authorization: Bearer admin-jwt-token-or-api-key" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Hello World Function",
...
```bash
curl -X PUT "https://your-app.apps.yarah.dev/api/functions/hello-world" \
  -H "Authorization: Bearer admin-jwt-token-or-api-key" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -X DELETE "https://your-app.apps.yarah.dev/api/functions/old-function" \
  -H "Authorization: Bearer admin-jwt-token-or-api-key"
```
