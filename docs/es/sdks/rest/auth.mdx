...
| `redirectTo` | string | No | Se utiliza para verificación de correo electrónico basada en enlace. El enlace de correo electrónico siempre abre primero un punto final del servidor Yarah; después de verificar el token, Yarah redirige el navegador a esta URL con el resultado de la verificación. Requerido cuando `verifyEmailMethod` es `link`. Esta URL debe incluirse en `allowedRedirectUrls`. Recomendado: utiliza la página de inicio de sesión de tu aplicación. |
...
- Yarah no crea un usuario al enviar el código. Si el correo es nuevo, crea un usuario verificado sin contraseña solo después de verificarlo correctamente.
...
- Éxito: `?yarah_status=success&yarah_type=verify_email`
- Error: `?yarah_status=error&yarah_type=verify_email&yarah_error=...`
- `yarah_status`: Resultado del flujo de enlace del navegador. Para verificación, los valores son `success` o `error`.
- `yarah_type`: Identificador de flujo. Para enlaces de verificación, esto es siempre `verify_email`.
- `yarah_error`: Presente solo cuando `yarah_status=error`. Mensaje de error legible por humanos para mostrar o registrar.
- Manejo recomendado: utiliza tu página de inicio de sesión como `redirectTo`. Cuando `yarah_status=success`, muestra un mensaje de confirmación y pide al usuario que inicie sesión con su correo electrónico y contraseña.
- Si `redirectTo` no está en la lista de permitidos, Yarah devuelve un error `400` cuyo mensaje incluye la URL rechazada, y `nextActions` te dice que la agregues a `allowedRedirectUrls`.
...
- Listo para restablecer: `?token=...&yarah_status=ready&yarah_type=reset_password`
- Error: `?yarah_status=error&yarah_type=reset_password&yarah_error=...`
- `token`: Presente solo cuando `yarah_status=ready`. Pasa este valor a `POST /api/auth/email/reset-password` como `otp`.
- `yarah_status`: Resultado del flujo de enlace del navegador. Para enlaces de restablecimiento, los valores son `ready` o `error`.
- `yarah_type`: Identificador de flujo. Para enlaces de restablecimiento, esto es siempre `reset_password`.
- `yarah_error`: Presente solo cuando `yarah_status=error`. Mensaje de error legible por humanos para mostrar o registrar.
- Tu aplicación solo debe renderizar el formulario de restablecimiento de contraseña cuando `yarah_status=ready` y `token` está presente.
...
<Note>
  ...
  `response_type` o `scope`; los valores generados por Yarah/proveedor ganan y los valores del cliente que entran en conflicto se ignoran.
</Note>
...
<Note>
El `yarah_code` es un código de autorización temporal que debe intercambiarse por tokens usando el punto final `/api/auth/oauth/exchange`.
</Note>
...
| Campo | Tipo | Requerido | Descripción |
|-------|------|----------|-------------|
| `code` | string | Sí | El `yarah_code` recibido en la devolución de llamada |
...
- Si `allowedRedirectUrls` está vacío, Yarah permite todos los redireccionamientos por conveniencia del desarrollador. Esto no es seguro para producción y debe evitarse fuera del desarrollo local.
...
| Campo | Tipo | Requerido | Descripción |
|-------|------|----------|-------------|
| `code` | string | Sí | Código de autorización o JWT de Yarah Cloud |
...
Actualizar token de acceso de administrador del panel. Este punto final utiliza la cookie httpOnly `yarah_admin_refresh_token` solo del panel y no comparte la cookie de actualización de aplicación/usuario.
