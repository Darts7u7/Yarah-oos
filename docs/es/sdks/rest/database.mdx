...
description: Operaciones de base de datos Postgres estilo PostgREST a través de la API REST de Yarah para consultas, filtros, joins y mutaciones desde cualquier cliente.
---
...
Obtiene el historial de migraciones personalizadas ejecutadas con éxito. Yarah las almacena en `system.custom_migrations`, pero el endpoint devuelve únicamente los metadatos de la migración y las sentencias ejecutadas. **Requiere autenticación de administrador.**
...
<Warning>
No incluyas `BEGIN`, `COMMIT` ni `ROLLBACK` dentro de una migración personalizada. Yarah ejecuta la migración dentro de su propia transacción.
</Warning>
