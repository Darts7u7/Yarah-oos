...
description: Suscríbete a canales, publica eventos, rastrea presencia y maneja reconexiones y actualizaciones de tokens vía Socket.IO con el SDK de TypeScript de Yarah.
---
...
El SDK de TypeScript abre una conexión Socket.IO a tu backend de Yarah. Te suscribes a canales con nombre, escuchas nombres de eventos y opcionalmente publicas eventos de vuelta a los canales a los que te has unido.
...
- Clientes que llaman a `yarah.realtime.publish(channel, event, payload)`.
...
yarah.realtime.on('connect', () => {
  console.log('Connected:', yarah.realtime.socketId);
});

yarah.realtime.on('error', (error) => {
  console.error(error.code, error.message);
});

await yarah.realtime.connect();

const subscription = await yarah.realtime.subscribe('order:123');

if (!subscription.ok) {
  throw new Error(subscription.error.message);
}

yarah.realtime.on('status_changed', (message) => {
...
```typescript
const response = await yarah.realtime.subscribe('chat:room-1');
...
```typescript
yarah.realtime.on('new_message', (message) => {
...
```typescript
yarah.realtime.once('checkout_completed', (message) => {
...
yarah.realtime.on('status_changed', handleStatus);
yarah.realtime.off('status_changed', handleStatus);
```
...
```typescript
yarah.realtime.unsubscribe('chat:room-1');
```
...
```typescript
import type { SocketMessage } from '@yarahdev/sdk';
...
yarah.realtime.on<OrderStatusMessage>('status_changed', (message) => {
...
```typescript
const members = yarah.realtime.getPresenceState('chat:room-1');
```
...
```typescript
import type { PresenceSyncEvent } from '@yarahdev/sdk';

yarah.realtime.on<PresenceSyncEvent>('presence:sync', ({ channel, presence }) => {
...
```typescript
yarah.realtime.on('connect_error', (error) => {
  console.error('Connection failed:', error.message);
});

yarah.realtime.on('disconnect', (reason) => {
...
await yarah.realtime.publish(channel, 'customer_viewed', {
  orderId,
