...
description: Crea órdenes y suscripciones de Razorpay desde una aplicación TypeScript con el SDK de Yarah, incluyendo webhooks manuales y cumplimiento de eventos.
---
...
- `yarah.payments.razorpay.createOrder(...)`
- `yarah.payments.razorpay.verifyOrder(...)`
- `yarah.payments.razorpay.createSubscription(...)`
- `yarah.payments.razorpay.verifySubscription(...)`
- `yarah.payments.razorpay.cancelSubscription(...)`
- `yarah.payments.razorpay.pauseSubscription(...)`
- `yarah.payments.razorpay.resumeSubscription(...)`
...
El SDK llama a las rutas de tiempo de ejecución de Razorpay con el token de usuario actual de Yarah. No llames a estas rutas con claves secretas del proveedor.
...
Crea primero una orden pendiente propiedad de la aplicación. Luego crea la orden de Razorpay a través de Yarah:

Las órdenes de Razorpay pueden ser solo de importe, pero crear artículos de Razorpay para productos vendibles de una sola vez es una buena práctica porque los artículos sincronizados hacen que el catálogo sea visible en Yarah y Razorpay. Trata las órdenes como intentos de pago.
...
const { data: orderResponse, error } =
  await yarah.payments.razorpay.createOrder('test', {
    amount: 50000,
    currency: 'INR',
    receipt: order.id,
...
const checkout = new RazorpayCheckout({
  ...
  handler: async (response: {
    ...
    const { error } = await yarah.payments.razorpay.verifyOrder('test', {
...
Crea o sincroniza primero un plan de Razorpay. Luego crea la suscripción a través de Yarah:
...
const { data: subscriptionResponse, error } =
  await yarah.payments.razorpay.createSubscription('test', {
    planId: 'plan_123',
    totalCount: 12,
    subject: { type: 'team', id: teamId },
    customerEmail: user.email
...
const checkout = new RazorpayCheckout({
  ...
  handler: async (response: {
    ...
    const { error } = await yarah.payments.razorpay.verifySubscription('test', {
