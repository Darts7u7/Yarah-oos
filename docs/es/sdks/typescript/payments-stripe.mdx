...
description: Crea sesiones de Stripe Checkout y Portal de Facturación con el SDK de TypeScript de Yarah para gestionar pagos, suscripciones y cumplimiento de webhooks.
---
...
- `yarah.payments.stripe.createCheckoutSession(...)`
- `yarah.payments.stripe.createCustomerPortalSession(...)`
...
npx @yarah/cli payments stripe catalog --environment test
```
...
Los métodos del SDK requieren el token de usuario actual de Yarah. Los tokens anónimos de Yarah se pueden usar para el pago único de invitados, pero una clave de API de Yarah no es un sustituto porque el backend necesita el contexto del usuario para las filas de sesión locales.

## createCheckoutSession()

Crea una sesión de Stripe Checkout a través del backend de Yarah.
...
<Warning>
  Las claves de metadatos que comienzan con `yarah_` están reservadas.
</Warning>
...
Si un pago único incluye un `subject` y aún no existe un mapeo de cliente de Stripe, Yarah permite que Stripe cree el cliente durante el Checkout y rellena el mapeo del sujeto desde el webhook de finalización.
...
Los métodos del SDK llaman a las rutas de tiempo de ejecución usando el token actual de Yarah. El backend inserta filas de sesión locales usando el contexto del llamante:
...
<Warning>
  Nunca pongas claves secretas de Stripe en el código del frontend ni en variables de entorno de despliegue público. Configura las claves de Stripe a través del panel de Yarah o la CLI.
</Warning>
