...
description: Autentica usuarios, gestiona perfiles, sesiones JWT y OAuth con el SDK de TypeScript de Yarah para Node, navegadores y tiempos de ejecución edge.
---
...
<Note>
  Cuando `requireEmailVerification` es verdadero, `accessToken` será nulo hasta que el usuario verifique su correo electrónico. Yarah envía un correo de verificación con un enlace o código de 6 dígitos, según la configuración del panel (`verifyEmailMethod`). Para verificación por código, implementa una página que pida al usuario ingresar el código (ver [verifyEmail()](#verifyemail)). Para verificación por enlace, proporciona una URL `redirectTo` que debería recibir el navegador después de que Yarah verifique el token. Recomendado: usa tu página de inicio de sesión como `redirectTo`, luego muestra un mensaje de éxito y pide al usuario que inicie sesión con su correo electrónico y contraseña.
</Note>
...
<Note>
  Después del redireccionamiento OAuth, el SDK detecta automáticamente el `yarah_code` de devolución de llamada, lo intercambia por una sesión y guarda la sesión automáticamente.
</Note>
...
<Note>
  `additionalParams` es solo para pistas opcionales específicas del proveedor. No pases campos OAuth propiedad del servidor como `client_id`, `redirect_uri`, `code_challenge`, `state`, `response_type` o `scope`; Yarah establece esos valores en el servidor e ignora las claves proporcionadas por el cliente en conflicto.
</Note>

<Note>
  Los proveedores personalizados deben configurarse primero en el panel de Yarah bajo `Auth Methods` con credenciales de cliente y una URL de descubrimiento OIDC.
</Note>
...
Manejo recomendado: usa tu página de inicio de sesión como `redirectTo`. Cuando `yarah_status=success`, muestra un mensaje de confirmación y pide al usuario que inicie sesión con su correo electrónico y contraseña.
...
- `token`: Presente solo cuando `yarah_status=ready`. Pasa este valor a `resetPassword({ otp })`.
...
Solo renderiza el formulario de restablecimiento de contraseña cuando `yarah_status=ready` y `token` está presente.
