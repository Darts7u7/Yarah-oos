...
description: Autentica usuarios, gestiona perfiles, sesiones JWT y OAuth con el SDK de Kotlin de Yarah para aplicaciones Android y proyectos Kotlin Multiplataforma.
---
...
Para los usuarios que se registran con correo electrónico, el backend de Yarah proporciona tres opciones:
...
- `redirectUri` (`String`) - URL de devolución de llamada donde Yarah redirigirá después de la autenticación
...
Al crear el cliente de Yarah, configura el `browserLauncher` para manejar la apertura de URLs:
...
`client` debe provenir de la misma instancia compartida del cliente de Yarah que utilizas en otros lugares de la aplicación, como un singleton de `Application` o tu contenedor de inyección de dependencias.
...
Yarah admite dos métodos de restablecimiento de contraseña, configurados en el backend:
