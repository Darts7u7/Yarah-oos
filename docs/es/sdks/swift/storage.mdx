...
description: Carga, descarga y gestiona archivos con el SDK de Swift de Yarah, con cubos por proyecto, URLs firmadas y políticas de acceso a nivel de fila en Apple.
---
...
```swift
let bucket = yarah.storage.from("images")
```
...
try await yarah.storage.createBucket("avatars")

// Create a private bucket
try await yarah.storage.createBucket(
    "documents",
...
```swift
let buckets = try await yarah.storage.listBuckets()
...
```swift
try await yarah.storage.updateBucket(
    "documents",
...
```swift
try await yarah.storage.deleteBucket("old-bucket")
```
...
let file = try await yarah.storage
    .from("images")
    .upload(
...
```swift
// Download file
let data = try await yarah.storage
    .from("images")
...
```swift
// List all files
let files = try await yarah.storage
    .from("images")
    .list()

// List with prefix filter
let userFiles = try await yarah.storage
    .from("images")
...
if let post = posts.first, let imageKey = post.imageKey {
    ...
    let _: [Post] = try await yarah.database
        .from("posts")
        .eq("id", value: "post-123")
...
```swift
let strategy = try await yarah.storage
    .from("videos")
    .getUploadStrategy(
