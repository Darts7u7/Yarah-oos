...
description: Invoca funciones sin servidor de Yarah desde Swift con el SDK oficial para ejecutar lógica de servidor, webhooks y tareas asíncronas en apps de Apple.
---
...
<Note>
Actualmente, Yarah solo admite funciones JavaScript/TypeScript ejecutándose en un entorno Deno.
</Note>
...
let response: HelloResponse = try await yarah.functions.invoke(
    "hello-world",
...
let response: GreetingResponse = try await yarah.functions.invoke(
    "hello-world",
    body: request
)
...
```swift
do {
    let response: MyResponse = try await yarah.functions.invoke(
        "my-function",
        body: ["key": "value"]
    )
    print("Success: \\(response)")
} catch let error as YarahError {
    switch error {
