...
description: Conecta vía Socket.IO, suscríbete a canales, publica eventos, rastrea presencia y maneja reconexiones con el cliente en tiempo real de Swift de Yarah.
---
...
El SDK de Swift se conecta a Yarah Realtime a través de Socket.IO. Suscríbase a nombres de canal como `order:123` o `chat:room-1`, luego escuche nombres de eventos publicados en esos canales.
...
```swift
try await yarah.realtime.connect()

await yarah.realtime.subscribe(to: "order:\\(orderId)") { message in
...
```swift
await yarah.realtime.subscribe(to: "chat:room-1") { message in
...
```swift
try await yarah.realtime.publish(
    to: "chat:room-1",
    event: "new_message",
    payload: [
...
```swift
await yarah.realtime.unsubscribe(from: "chat:room-1")
```
...
```swift
await yarah.realtime.disconnect()
```
...
```swift
let channel = await yarah.realtime.channel("chat:room-1")
...
```swift
await yarah.realtime.removeChannel("chat:room-1")
```
