...
description: Usa Yarah como proveedor de identidad OAuth 2.0 para autenticar usuarios en aplicaciones de terceros, agentes de IA y clientes MCP con permisos limitados.
---

## Descripción general

Yarah puede funcionar como proveedor de identidad OAuth 2.0, permitiendo que aplicaciones de terceros autentiquen usuarios mediante "Sign in with Yarah". Esto permite que los desarrolladores que construyen sobre tu plataforma aprovechen el sistema de autenticación de Yarah sin tener que gestionar sus propias credenciales de usuario.

## Casos de uso

<CardGroup cols={2}>
  <Card title="Plataformas para desarrolladores" icon="code">
    Permite que desarrolladores externos creen integraciones con "Sign in with Yarah", mientras mantienes el control sobre el acceso a los datos de los usuarios.
  </Card>
  ...
  <Card title="Aplicaciones de socios" icon="handshake">
    Permite que las aplicaciones de socios autentiquen usuarios contra tu proyecto de Yarah sin compartir credenciales.
  </Card>
...
Yarah implementa el **flujo de código de autorización con PKCE** (Proof Key for Code Exchange), el flujo OAuth más seguro tanto para aplicaciones web como nativas.

```mermaid
sequenceDiagram
    participant App as Your Application
    participant User as User Browser
    participant Yarah as Yarah Auth
    participant API as Yarah API

    App->>App: Generate code_verifier & code_challenge
    App->>User: Redirect to /api/oauth/v1/authorize
    User->>Yarah: User authenticates
    Yarah->>User: Authorization prompt
    User->>Yarah: User approves
    Yarah->>App: Redirect with authorization code
    App->>Yarah: POST /api/oauth/v1/token<br/>(code + code_verifier)
    Yarah->>Yarah: Verify PKCE
    Yarah->>App: Access token + Refresh token
...
<Steps>
  <Step title="Registra tu aplicación">
    Contacta a Yarah para registrar tu aplicación como un cliente OAuth. Recibirás:
...
```
GET https://api.yarah.dev/api/oauth/v1/authorize
```
...
```
https://api.yarah.dev/api/oauth/v1/authorize?
...
```
POST https://api.yarah.dev/api/oauth/v1/token
```
...
```
GET https://api.yarah.dev/auth/v1/profile
...
YARAH_CLIENT_ID=clf_your_client_id
YARAH_CLIENT_SECRET=your_client_secret
YARAH_URL=https://api.yarah.dev
...
const requiredEnvVars = ['SESSION_SECRET', 'YARAH_CLIENT_ID', 'YARAH_CLIENT_SECRET'];
...
const config = {
  ...
  yarahUrl: process.env.YARAH_URL || 'https://api.yarah.dev',
...
app.get('/auth/login', (req, res) => {
  ...
  const authUrl = new URL(`${config.yarahUrl}/api/oauth/v1/authorize`);
...
app.get('/auth/callback', async (req, res) => {
  ...
  try {
    ...
    const tokenResponse = await fetch(`${config.yarahUrl}/api/oauth/v1/token`, {
      method: 'POST',
    ...
    // Fetch user profile
    const profileResponse = await fetch(`${config.yarahUrl}/auth/v1/profile`, {
...
app.get('/api/organizations', async (req, res) => {
  ...
  const response = await fetch(`${config.yarahUrl}/organizations/v1`, {
...
<Card title="Repositorio de ejemplo de OAuth" icon="github" href="https://github.com/Darts7u7/Yarah-oauth-example">
  Ejemplo completo y funcional que muestra cómo integrar "Sign in with Yarah" en tu aplicación.
</Card>
