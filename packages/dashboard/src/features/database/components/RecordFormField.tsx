...
function getPlaceholderText(field: ColumnSchema, t: TFunction): string {
  if (field.defaultValue && field.defaultValue.endsWith('()')) {
    return t('database.autoGeneratedOnSubmit', { defaultValue: 'Auto-generated on submit' });
  }
...
function FormDateEditor({
  ...
  const formatDisplayValue = () => {
    if (!value || value === 'null') {
      return getPlaceholderText(field, t);
    }
...
function FormNumberEditor({ value, type, onChange, tableName, field }: FormNumberEditorProps) {
  ...
  return (
    <Input
      ...
      placeholder={getPlaceholderText(field, t)}
          ...
          <div className="flex min-w-0 items-start gap-1.5">
            ...
            {hasLinkedValue && (
              <Button
                ...
                title={t('database.clearLinkedRecord', { defaultValue: 'Clear linked record' })}
              >
                <X className="h-4 w-4" />
              </Button>
            )}
            ...
            >
              {(openModal) => (
                <Button
                  ...
                  title={
                    hasLinkedValue
                      ? t('database.changeLinkedRecord', {
                          table: foreignKey.referenceTable,
                          defaultValue: 'Change linked {{table}} record',
                        })
                      : t('database.linkToRecord', {
                          table: foreignKey.referenceTable,
                          defaultValue: 'Link to {{table}} record',
                        })
                  }
                >
      ...
      <div className="min-w-0 space-y-1">
        ...
        {foreignKey && (
          <div className="flex min-w-0 items-center gap-1.5 text-xs text-muted-foreground">
            <span className="truncate">
              {t('database.hasForeignKeyRelationTo', {
                defaultValue: 'Has a Foreign Key relation to',
              })}
            </span>
            <FormMetaBadge>
        ...
        {errors[field.columnName] && (
          <p className="text-sm leading-5 text-destructive">
            {(errors[field.columnName] as FieldError)?.message ||
              t('database.fieldRequired', {
